...
DATA=json.loads((ROOT/'data/v317/intelligence.json').read_text(encoding='utf-8'));RUN=json.loads((ROOT/'data/v317/last_run.json').read_text(encoding='utf-8'));GAPS=json.loads((ROOT/'data/v317/research_gaps.json').read_text(encoding='utf-8'));METRICS=json.loads((ROOT/'data/v317/metrics_before_after.json').read_text(encoding='utf-8'));INDEX=(ROOT/'index.html').read_text(encoding='utf-8');JS=(ROOT/'assets/v317/intelligence.js').read_text(encoding='utf-8');CSS=(ROOT/'assets/v317/intelligence.css').read_text(encoding='utf-8')
...
class TestV317(unittest.TestCase):
 def test_01_version_everywhere(self):self.assertEqual((ROOT/'VERSION').read_text().strip(),'3.17.0');self.assertEqual(DATA['meta']['version'],'3.17.0');self.assertEqual(RUN['version'],'3.17.0');self.assertEqual(GAPS['version'],'3.17.0')
 ...
 def test_11_gap_fixed_definition(self):self.assertIn('Misma definición v3.15→v3.17',GAPS['definition']);self.assertGreater(GAPS['total_gaps'],0)
 def test_12_gap_has_48_passes(self):self.assertTrue(all(len(g['passes'])==48 for g in GAPS['gaps']))
 ...
 def test_21_gaps_reduced(self):self.assertLess(METRICS['after']['research_gaps'],METRICS['before']['research_gaps']);self.assertGreater(METRICS['gap_reduction_pct'],10)
 ...
 def test_24_reports_exist(self):
  for n in ('metrics_before_after.json','source_report.json','coverage_report.json','research_gaps.json'):self.assertTrue((ROOT/'data/v317'/n).is_file(),n)
 ...
 def test_32_frontend_version(self):self.assertIn("fetch('data/v317/intelligence.json'",JS);self.assertIn('assets/v317/intelligence.js?v=3.17.0',INDEX)
 def test_33_exports(self):self.assertIn('window.jspdf?.jsPDF',JS);self.assertIn('v3.17.0.pdf',JS);self.assertIn('v3.17.0.pptx',JS)
 def test_34_workflows(self):
  for n in ('research-daily.yml','research-weekly.yml','research-monthly.yml'):
   t=(ROOT/'.github/workflows'/n).read_text();self.assertIn('research_supervisor_v317.py',t);self.assertIn('tests/test_v317.py',t);self.assertIn('data/v317/',t)
 def test_35_pages(self):self.assertIn('cp -R data/v317 _site/data/v317',(ROOT/'.github/workflows/pages-deploy.yml').read_text())
 def test_36_responsive(self):self.assertIn('@media(max-width:900px)',CSS);self.assertIn('@media(max-width:620px)',CSS)
 def test_37_documentation(self):self.assertTrue((ROOT/'README_V317.md').exists());self.assertTrue((ROOT/'CHANGELOG_V317.md').exists())
 ...
 def test_41_ecosystem_columns_for_both_sections(self):
  for section in ('distributors','integrators'):
   ids={c['id'] for c in DATA['schemas'][section]};self.assertTrue({'westcon_overlap','competitor_vendor_overlap'}<=ids)
 def test_42_ecosystem_split_is_populated_and_traceable(self):
  for section,minimum in (('distributors',12),('integrators',120)):
   for fid in ('westcon_overlap','competitor_vendor_overlap'):
    rows=[r for r in DATA[section] if field(r,fid).get('value') and evidence_rows(field(r,fid))];self.assertGreaterEqual(len(rows),minimum,(section,fid))
 def test_43_publisher_is_version_dynamic(self):
  text=(ROOT/'scripts/publish_research_update.py').read_text(encoding='utf-8');self.assertIn('current_version()',text);self.assertNotIn('scripts/v312/validate_v312.py',text)
