...
const js = fs.readFileSync(path.join(root, 'assets/v382/intelligence.js'), 'utf8');
...
assert(html.includes('assets/v382/intelligence.js'), 'Falta JS v3.8.2');
assert(html.includes('assets/v382/intelligence.css'), 'Falta CSS v3.8.2');
...
assert(html.includes('dataStatusBtn') && html.includes('updateModal'), 'Falta estado visible de actualización automática');
assert(js.includes('data/v38/last_run.json') && js.includes('renderUpdateStatus'), 'Falta lectura del último ciclo automático');
assert(html.includes('confidenceHelpBtn') && html.includes('confidenceModal'), 'Falta ayuda global de confianza');
assert(js.includes('deriveConfidenceFactors') && js.includes('Por qué tiene este nivel'), 'Faltan razones explícitas de confianza');
assert(js.includes('Momentum (M)') && js.includes('quadrant-label'), 'Los gráficos no explican su interpretación');
assert(js.includes("context==='trend'") && js.includes('trend-card'), 'Las tarjetas de tendencias no usan compactación específica');
...
console.log('UI smoke v3.8.2 · PASS');
