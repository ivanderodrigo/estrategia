(() => {
  'use strict';
  const state = {data:null, lastRun:null, view:'fabricantes', fontScale:Number(localStorage.getItem('westcon-font-scale')||1), sort:{}, columnOrder:{}, dragCol:null};
  ...
  async function load(){
    const [res, runRes] = await Promise.all([
      fetch('data/v39/intelligence.json', {cache:'no-store'}),
      fetch('data/v39/last_run.json', {cache:'no-store'}).catch(()=>null)
    ]);
    if(!res.ok) throw new Error(`No se pudo cargar data/v39/intelligence.json (${res.status})`);
  ...
  function bind(){
    ...
    $('#dataStatusBtn')?.addEventListener('click', () => openModal('updateModal'));
    $('#confidenceHelpBtn')?.addEventListener('click', () => openModal('confidenceModal'));
  ...
  function pptAddFooter(slide,pptx,label,dark=false){
    ...
    slide.addText(`v${state.data.meta.version||'3.9.0'} · ${state.data.meta.scope||'Iberia'} · trazabilidad en la aplicación`,{x:7.0,y:7.1,w:5.75,h:.14,fontFace:'Aptos',fontSize:6.6,color,align:'right',margin:0});
  }
  ...
  async function exportPptx(){
    ...
    slide.addText('Fabricantes · Mayoristas · Integradores · Clientes · Tendencias · Arquitecturas',{x:.74,y:3.55,w:10.7,h:.32,fontFace:'Aptos',fontSize:11.5,color:'D2E0E6',margin:0});
    ...
    order.forEach(([key,rows,schema])=>{if(!rows?.length)return;pptAddDomainDivider(pptx,key,rows.length);if(key==='trends'||key==='architectures')pptAddDetailSlides(pptx,key,rows,schema);else pptAddEntitySlides(pptx,key,rows,schema);if(key==='trends')pptAddTrendAnalytics(pptx);selectedRows.push(...rows);});
    pptAddSources(pptx,selectedRows);
    await pptx.writeFile({fileName:'Westcon_Iberia_Business_Intelligence_v3.9.0.pptx'}); closeModal('exportModal'); toast('PowerPoint generado con diseño Westcon');
  }
  ...
  function renderAll(){
    ...
    const meta=state.data.meta||{}; const status=$('#footerStatus'); if(status) status.textContent=`App v3.9.0 · dataset v${meta.version||'3.9.0'} · ${meta.source_count||0} fuentes/familias · ${meta.scope||'Iberia'}`;
  }
