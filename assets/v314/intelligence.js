(() => {
  ...
  async function load(){
    const [res, runRes] = await Promise.all([
      fetch('data/v314/intelligence.json', {cache:'no-store'}),
      fetch('data/v314/last_run.json', {cache:'no-store'}).catch(()=>null)
    ]);
    if(!res.ok) throw new Error(`No se pudo cargar data/v314/intelligence.json (${res.status})`);
  ...
  function activeColumns(schema, rows, view){
    let cols=schema.filter(col=>{
      ...
      if(col.decision_required) return true;
      if(!populated) return false;
      if(view==='cards') return true;
      // Los campos opcionales muy dispersos se ocultan hasta tener cobertura útil.
      // Los campos decisionales permanecen visibles aunque todavía falte evidencia.
      const minimum=rows.length>=50?Math.max(8,Math.ceil(rows.length*.20)):1;
  ...
  function missingMarkup(col){
    if(col?.empty_mode==='critical'||col?.decision_required) return '<span class="research-gap critical-gap">Pendiente de evidencia</span>';
    return '<span class="no-public-data" title="Sin evidencia pública suficiente o campo no aplicable">—</span>';
  }
  ...
  function reportFooter(label){return `<footer class="r-footer"><span>${esc(label)}</span><span>v${esc(state.data.meta.version||'3.14.0')} · ${esc(state.data.meta.scope||'Iberia')} · inteligencia trazable</span></footer>`;}
  ...
  function reportCover(title,modules){
    ...
    return `<section class="report-page report-cover">
      <div class="r-cover-top">${reportBrand()}<span class="r-version">v${esc(state.data.meta.version||'3.14.0')}</span></div>
  ...
  function pptAddFooter(slide,pptx,label,dark=false){
    ...
    slide.addText(`v${state.data.meta.version||'3.14.0'} · ${state.data.meta.scope||'Iberia'} · trazabilidad en la aplicación`,{x:7.0,y:7.1,w:5.75,h:.14,fontFace:'Aptos',fontSize:6.6,color,align:'right',margin:0});
  }
  ...
  async function exportPptx(){
    ...
    await pptx.writeFile({fileName:'Westcon_Iberia_Business_Intelligence_v3.14.0.pptx'});closeModal('exportModal');toast(appendix?'PowerPoint ejecutivo + anexo generado':'PowerPoint ejecutivo generado');
  }
  ...
  function renderAll(){
    ...
    const meta=state.data.meta||{}; const status=$('#footerStatus'); if(status) status.textContent=`App v3.14.0 · dataset v${meta.version||'3.14.0'} · ${meta.source_count||0} fuentes/familias · ${meta.scope||'Iberia'}`;
  }
