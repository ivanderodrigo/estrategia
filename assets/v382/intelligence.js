(() => {
  ...
  async function load(){
    const [res, runRes] = await Promise.all([
      fetch('data/v38/intelligence.json', {cache:'no-store'}),
      fetch('data/v38/last_run.json', {cache:'no-store'}).catch(()=>null)
    ]);
  ...
  async function exportPptx(){
    ...
    await pptx.writeFile({fileName:'Westcon_Iberia_Business_Intelligence_v3.8.2.pptx'}); closeModal('exportModal'); toast('PowerPoint generado con diseño Westcon');
  }

  function renderAll(){
    populateIntegratorVendorFilter();
    renderManufacturers(); renderIntegrators(); renderDistributors(); renderTrends(); renderArchitectures(); renderSourceCatalog(); renderUpdateStatus(); renderConfidenceGuide();
    const meta=state.data.meta||{}; const status=$('#footerStatus'); if(status) status.textContent=`App v3.8.2 · dataset v${meta.version||'3.8.0'} · ${meta.source_count||0} fuentes/familias · ${meta.scope||'Iberia'}`;
  }
