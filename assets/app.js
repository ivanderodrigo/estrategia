...
async function load(){
  const [data,base,research,reality,graph,engine,decision,capability,ecosystem,universe,schedule,discoveredEntities,sourceHealth,researchErrors,runManifest,supervisor,status,changes]=await Promise.all([
  ...
  state.data=data;state.base=base;state.research=research;state.reality=reality;state.graph=graph;state.engine=engine;state.decision=decision;state.capability=capability;state.ecosystem=ecosystem;state.universe=universe;state.schedule=schedule;state.discoveredEntities=discoveredEntities;state.sourceHealth=sourceHealth;state.researchErrors=researchErrors;state.runManifest=runManifest;state.supervisor=supervisor;state.status=status;state.changes=changes;state.reportModules=new Set((decision.exportModules||[]).filter(x=>x.default).map(x=>x.id));
...
function enrichVendor(v){
  ...
  const baseRecommendation=recommendationFor(analysis),stability=decisionStability(v,analysis,baseRecommendation),reality=realityFor(v);analysis.decisionStability=stability.score;analysis.recommendationAlternatives=stability.alternatives;analysis.baseRecommendation=baseRecommendation;analysis.computedRecommendation=robustRecommendation(baseRecommendation,analysis,stability.score);analysis.actionAuthorized=Boolean(reality&&Number(reality.confidence)===100&&realityEvidence(v).length>=3);analysis.recommendation=analysis.actionAuthorized?reality.decision:'INVESTIGAR';analysis.recommendationSource=analysis.actionAuthorized?'CERTAINTY_100_GATE':'BLOCKED_BELOW_100';analysis.recommendationConfidence=reality?.confidence||reliability;
...
function renderDataHealth(){
  const items=[['Acciones autorizadas',state.vendors.filter(v=>v.analysis.actionAuthorized).length,state.vendors.length,'Solo certeza 100/100'],['Canal ES/PT',state.vendors.filter(v=>v.derived.channels.length).length,state.vendors.length,'Mayoristas alternativos identificados'],['Integradores',state.vendors.filter(v=>v.derived.integrators.length).length,state.vendors.length,'Partners/integradores con prueba pública'],['Clientes públicos',state.vendors.filter(v=>v.derived.customers.length).length,state.vendors.length,'Referencias finales ES/PT'],['Demanda pública',state.vendors.filter(v=>v.analysis.publicDemand>=35).length,state.vendors.length,'Señales estructuradas de contratación ES/PT'],['Consultoras',state.vendors.filter(v=>v.analysis.analystSignal>=50).length,state.vendors.length,'Señal pública específica o de mercado'],['Evidencia fuerte',state.vendors.filter(v=>v.analysis.evidenceConfidence>=65).length,state.vendors.length,'Cobertura suficiente para lectura ejecutiva']];
...
const VENDOR_COLUMNS={
  ...
  decision:{label:'Estado',sort:v=>v.analysis.actionAuthorized?100:v.analysis.recommendationConfidence,text:v=>v.analysis.actionAuthorized?v.analysis.recommendation:'SIN ACCIÓN',html:v=>derivedIntel(`<span class="priority ${v.analysis.recommendation}">${v.analysis.actionAuthorized?v.analysis.recommendation:'SIN ACCIÓN'}</span>`,'Gate de decisión',v,v.analysis.actionAuthorized?'Certeza 100/100 y contrato de ejecución':'Acción bloqueada por debajo de certeza 100/100')},
  ...
  gate:{label:'Gate / siguiente paso',sort:v=>v.analysis.actionAuthorized?100:0,text:v=>v.analysis.actionAuthorized?v.derived.plan.p30:'Bloqueada hasta certeza 100/100',html:v=>v.analysis.actionAuthorized?esc(v.derived.plan.p30):'<span class="tiny">Bloqueada &lt;100</span>'}
};
...
function realityContractHtml(v){
  const r=realityFor(v),p=v.derived.plan;
  if(!v.analysis.actionAuthorized){const ready=gateReadiness(v);return `<section class="reality-contract blocked"><div class="contract-head"><span>ACCIÓN NO AUTORIZADA · ${r?.confidence||v.analysis.recommendationConfidence}/100</span><b>El umbral ejecutivo es 100/100</b></div><p>${esc(r?.thesis||'La evidencia disponible sirve para analizar, pero no justifica todavía una acción.')}</p><p>Falta corroboración total y/o el gate interno. Estado interno: ${ready.answered}/${ready.total}. La aplicación no convierte una inferencia en mandato.</p><div class="contract-gate"><b>Condición de salida</b><span>${esc(p.goNoGo)}</span></div></section>`}
  const sources=realityEvidence(v).map(e=>`<a href="${e.url}" target="_blank" rel="noopener">${esc(e.source)} · ${esc(e.title)} ↗</a>`).join('');
  return `<section class="reality-contract"><div class="contract-head"><span>DECISIÓN EJECUTIVA VERIFICADA · CONFIANZA ${r.confidence}/100</span><b>${esc(r.decision)} · ${esc(p.owner)}</b></div><p class="contract-thesis">${esc(r.thesis)}</p><div class="contract-timeline"><div><b>30 días</b><p>${esc(p.p30)}</p></div><div><b>60 días</b><p>${esc(p.p60)}</p></div><div><b>90 días</b><p>${esc(p.p90)}</p></div></div><div class="contract-kpis"><b>Resultado medible</b>${(p.kpis||[]).map(x=>`<span>${esc(x)}</span>`).join('')}</div><div class="contract-gate"><b>GO / NO-GO</b><span>${esc(p.goNoGo)}</span></div><details><summary>${realityEvidence(v).length} fuentes públicas que sostienen esta decisión</summary>${sources}</details></section>`
}
function selectVendor(name){
  ...
  <div class="action-box"><b>${a.actionAuthorized?'ACCIÓN AUTORIZADA':'LECTURA, NO ACCIÓN'}</b><p>${a.actionAuthorized?esc(v.derived.plan.p90):'El motor conserva la tesis y los datos, pero bloquea la recomendación hasta alcanzar 100/100.'}</p></div>
  ${internalGateHtml(v)}
  ${a.actionAuthorized?`<div class="detail-block"><h4>ACCIONES PRIORITARIAS CROSS-FUNCTION</h4><div class="action-stack">${execActs}</div></div><div class="detail-block"><div class="role-picker"><h4>RECOMENDACIONES POR PERFIL</h4><select id="roleSelect">${roleOpts}</select></div><div id="roleRecommendations">${roleRecommendationHtml(v,state.selectedRole)}</div></div>`:'<div class="method-note"><b>Acciones ocultas.</b> Los scores internos ayudan a priorizar investigación, nunca a justificar ejecución por debajo de 100/100.</div>'}
...
function renderEngine(){
  ...
  const w=state.engine.opportunityWeights,u=state.universe||{};$('#engineExplanation').innerHTML=`<p><b>Motor v11 acotado, adaptativo y reanudable.</b> Investiga ${u.analysts?.length||0} consultoras, ${u.distributors?.length||0} mayoristas y ${u.integrators?.length||0} integradores semilla, además de actores descubiertos dinámicamente. Cada ejecución usa un presupuesto estricto de tiempo: procesa lotes pequeños, publica lo válido, conserva la cola pendiente y continúa en el ciclo siguiente.</p><p>Aprende autoridad, disponibilidad, latencia, rendimiento, precisión ES/PT, relación nueva, cifras, corroboración y frescura por consulta y dominio. Abre circuitos ante fallos repetidos, rota el universo para evitar sesgos y conserva el último dataset válido si el proceso no supera validación.</p><p><b>Gate ejecutivo:</b> el motor puede puntuar hipótesis, pero la interfaz solo muestra una acción si alcanza exactamente 100/100 con evidencia corroborada. Las demás quedan en investigación.</p><div class="weight-list">${Object.entries(w).map(([k,v])=>`<div><span>${esc(labels[k]||k)}</span><b>${Math.round(v*100)}%</b><i style="--w:${v*100}%"></i></div>`).join('')}</div><p class="tiny">Evidencia pública y datos internos permanecen separados. Los datos ausentes se muestran como gaps; nunca se estiman en silencio.</p>`
}
...
function reportHtml(){
  ...
  if(moduleOn('executive'))parts.push(`<section class="report-module"><div class="report-section-head"><h2>Resumen ejecutivo</h2><small>${authorized.length} acciones autorizadas de ${state.vendors.length}</small></div><p>Una acción solo aparece si alcanza certeza 100/100, dispone de evidencia pública corroborada y contrato de ejecución. Las puntuaciones inferiores orientan investigación, no inversión.</p>${authorized.length?authorized.slice(0,6).map(v=>`<h3>${esc(v.name)} · ${esc(v.analysis.recommendation)}</h3><p><b>Confianza 100/100 · Owner: ${esc(v.derived.plan.owner)}</b><br>${esc(v.derived.plan.thesis)}<br><b>30:</b> ${esc(v.derived.plan.p30)}<br><b>60:</b> ${esc(v.derived.plan.p60)}<br><b>90:</b> ${esc(v.derived.plan.p90)}<br><b>Gate:</b> ${esc(v.derived.plan.goNoGo)}</p>`).join(''):'<div class="method-note"><b>No hay acciones autorizadas.</b> El informe conserva las lecturas y gaps prioritarios, pero no presenta inferencias como mandatos.</div>'}</section>`);
...
async function exportPptx(){
  ...
  if(moduleOn('methodology')){s=pptx.addSlide('MASTER');addTitle(s,'Metodología','Simplicidad visible; evidencia, actualización y fallos trazables.');s.addText('14 dimensiones estratégicas\n+ aprendizaje por rendimiento de fuente\n+ presupuesto estricto de tiempo\n+ checkpoint reanudable\n+ corroboración y geografía\n+ gate de certeza 100/100\n+ diagnóstico por etapa',{x:.65,y:1.8,w:4.0,h:3.7,fontFace:'Aptos Display',fontSize:18,bold:true,color:'082335',margin:0,breakLine:true});s.addText('El motor v11 no confunde score con recomendación. Publica resultados parciales válidos, mantiene una cola pendiente, conserva el último dataset correcto si una ejecución falla y registra etapa, dominio, latencia, tipo de error y recuperación. Los datos internos permanecen separados y requieren revisión humana.',{x:5.0,y:1.8,w:7.3,h:3.3,fontFace:'Aptos',fontSize:14,color:'445D6C',margin:0})}
  await pptx.writeFile({fileName:'Westcon_Iberia_Decision_Intelligence_v3.pptx'});toast('PowerPoint ejecutivo generado')
}
