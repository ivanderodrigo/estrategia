...
const state={data:null,base:null,research:null,reality:null,graph:null,engine:null,decision:null,capability:null,ecosystem:null,status:null,changes:null,vendors:[],selected:null,selectedRole:'country_manager',quick:'all',deep:false,fontScale:1,reportModules:new Set()};
...
async function load(){
  const [data,base,research,reality,graph,engine,decision,capability,ecosystem,status,changes]=await Promise.all([
...
function renderAll(){state.fontScale=Number(localStorage.getItem('westcon-font-scale')||1);document.documentElement.style.setProperty('--font-scale',state.fontScale);renderKpis();renderDecisionCards();renderChanges();renderDataHealth();renderFilters();renderOverviewVendors();renderVendorTable();renderDistributors();renderIntegrators();renderArchitectures();renderTrends();renderSignals();renderDataKpis();renderEngine();renderGaps();renderSources();renderResearch()}
...
function renderDistributors(){const root=$('#distributorCards');if(!root)return;root.innerHTML=(state.graph?.distributors||[]).map(d=>`<article class="entity-card"><div class="entity-head"><div><span>${esc(d.countries.join(' + '))}</span><h3>${esc(d.name)}</h3></div><b>${d.scaleClaimIds?.length?'DATOS PÚBLICOS':'GAPS ABIERTOS'}</b></div><p>${esc(d.positioning)}</p><div class="claim-grid">${entityClaimHtml(d.scaleClaimIds)}</div><h4>Solape con Westcon</h4><p>${esc(d.overlap)}</p><h4>Ventajas demostrables / declaradas</h4><div>${tags(d.advantages)}</div><div class="gap-note"><b>No inventar</b>${esc(d.gaps.join(' · '))}</div></article>`).join('')}
function renderIntegrators(){const root=$('#integratorCards');if(!root)return;root.innerHTML=(state.graph?.integrators||[]).map(d=>`<article class="entity-card"><div class="entity-head"><div><span>${esc(d.countries.join(' + '))}</span><h3>${esc(d.name)}</h3></div><b>${d.claimIds?.length?'VERIFICADO':'EN INVESTIGACIÓN'}</b></div><div>${tags(d.specializations,'synergy')}</div><div class="claim-grid">${entityClaimHtml(d.claimIds)}</div><h4>Portfolio públicamente asociado</h4><div>${tags(d.vendors,'',9)}</div><div class="gap-note"><b>Pendiente</b>${esc(d.gaps.join(' · '))}</div></article>`).join('')}
...
function renderVendorTable(){
  ...
  $('#vendorRows').innerHTML=rows.map(v=>`<tr data-vendor="${esc(v.name)}" class="${state.selected===v.name?'selected':''}"><td><span class="vendor-name">${esc(v.name)}</span><span class="tiny">${v.countries.join(' · ')} · ${esc(v.domain)}</span></td><td><span class="priority ${v.analysis.recommendation}">${v.analysis.actionAuthorized?v.analysis.recommendation:'SIN ACCIÓN'}</span></td><td>${scoreCell(v,'opportunity')}</td><td>${tags(v.marketCompetitors,'',3)}</td><td>${channelSummary(v)}</td><td>${ecosystemSummary(v)}</td><td>${analystSummary(v)}</td><td>${scoreCell(v,'synergyPotential','good')}</td><td>${scoreCell(v,'overlapRisk','bad')}</td><td><span class="confidence-dot ${v.analysis.evidenceConfidence>=65?'high':v.analysis.evidenceConfidence>=45?'mid':'low'}"></span>${v.analysis.evidenceConfidence}<span class="tiny">${evidenceLabel(v)}</span></td><td>${v.analysis.actionAuthorized?esc(v.derived.plan.p30):'Bloqueada <100'}</td></tr>`).join('');
...
function renderEngine(){
  ...
  const w=state.engine.opportunityWeights;$('#engineExplanation').innerHTML=`<p><b>Motor v10 adaptativo y reanudable.</b> Lanza búsquedas masivas en lotes acotados, reintenta errores transitorios, abre circuitos para una fuente inestable y guarda un checkpoint atómico tras cada lote. Aprende con recompensas por fuente primaria, precisión ES/PT, relación nueva, cifra extraída, corroboración y frescura; penaliza duplicados y discovery sin validar. Mantiene exploración mínima para no encerrarse en las fuentes conocidas.</p><p><b>Gate ejecutivo:</b> el catálogo interno puede puntuar hipótesis, pero la interfaz solo muestra una acción si alcanza exactamente 100/100 y dispone de corroboración y gate interno. Hoy las demás quedan bloqueadas.</p><div class="weight-list">${Object.entries(w).map(([k,v])=>`<div><span>${esc(labels[k]||k)}</span><b>${Math.round(v*100)}%</b><i style="--w:${v*100}%"></i></div>`).join('')}</div><p class="tiny">Evidencia pública y datos internos permanecen separados. Los datos ausentes se muestran como gaps; nunca se estiman en silencio.</p>`
}
...
Autoactualización: diaria + profunda semanal + barrido exhaustivo mensual. Ejecución incremental, reanudable y tolerante a fallos por motor.
...
function reportHtml(){
  const top=[...state.vendors].sort(verifiedDecisionSort).slice(0,15),parts=[];
  parts.push(`<div class="report-export"><div style="border-top:8px solid #f09e0d;padding-top:18px"><div style="font-size:10px;color:#3195bb;font-weight:800">WESTCON IBERIA · FY27–FY30</div><h1>Radar Estratégico Tecnológico</h1><p class="note">Motor v9 · Evidence-to-Action pública + portfolio FY27 facilitado. Realidad, responsable, hitos, KPI, gate y fuentes.</p></div>`);
  if(moduleOn('executive'))parts.push(`<section class="report-module"><h2>Resumen ejecutivo</h2><p>Una decisión solo se presenta como verificada si enlaza realidad pública con ejecución y condición de salida. Los demás fabricantes quedan en investigación.</p>${top.slice(0,6).map(v=>`<h3>${esc(v.name)} · ${esc(v.analysis.recommendation)}</h3><p><b>Confianza ${v.analysis.recommendationConfidence}/100 · Owner: ${esc(v.derived.plan.owner)}</b><br>${esc(v.derived.plan.thesis)}<br><b>30:</b> ${esc(v.derived.plan.p30)}<br><b>60:</b> ${esc(v.derived.plan.p60)}<br><b>90:</b> ${esc(v.derived.plan.p90)}<br><b>Gate:</b> ${esc(v.derived.plan.goNoGo)}</p>`).join('')}</section>`);
  if(moduleOn('portfolio'))parts.push(`<section class="report-module"><h2>Prioridades por fabricante</h2><table><thead><tr><th>Fabricante</th><th>Tesis</th><th>Decisión</th><th>Confianza</th><th>Oportunidad</th><th>Vendor</th><th>Ecosistema</th><th>Canal</th><th>Solape</th><th>Owner</th><th>Acción 90 días</th></tr></thead><tbody>${top.map(v=>`<tr><td>${esc(v.name)}</td><td>${esc(v.derived.archetype?.name||'')}</td><td>${v.analysis.recommendation}</td><td>${v.analysis.recommendationConfidence}</td><td>${v.analysis.opportunity}</td><td>${v.analysis.vendorMomentum}</td><td>${v.analysis.ecosystemStrength}</td><td>${v.analysis.channelPressure}</td><td>${v.analysis.overlapRisk}</td><td>${esc(v.derived.plan.owner)}</td><td>${esc(v.derived.plan.p90)}</td></tr>`).join('')}</tbody></table></section>`);
  ...
  if(moduleOn('methodology'))parts.push(`<section class="report-module"><h2>Metodología</h2><p>Evidence-to-Action v9: mercado, competencia, ecosistema y capacidades Westcon verificadas. Cada recomendación exige realidad, acción 30/60/90, owner, KPI, go/no-go y fuentes. Las 47 palancas pasan una matriz obligatoria fabricante × capacidad × ámbito × evidencia. Los datos internos del gate quedan solo en el navegador y no alteran la puntuación pública.</p></section>`);
  parts.push('</div>');return parts.join('')
}
async function exportPdf(){if(!window.html2pdf){toast('Librería PDF no disponible');return}const report=$('#report');report.innerHTML=reportHtml();report.style.display='block';await html2pdf().set({margin:8,filename:'Westcon_Iberia_Evidence_to_Action_v2.0.pdf',image:{type:'jpeg',quality:.96},html2canvas:{scale:1.4,useCORS:true},jsPDF:{unit:'mm',format:'a4',orientation:'landscape'},pagebreak:{mode:['css','legacy']}}).from(report.firstElementChild).save();report.style.display='none';toast('PDF generado')}
async function exportPptx(){
  if(!window.PptxGenJS){toast('Librería PowerPoint no disponible');return}const pptx=new PptxGenJS();pptx.layout='LAYOUT_WIDE';pptx.author='Westcon Iberia Strategy Studio';pptx.title='Westcon Iberia · Evidence-to-Action v2.0';pptx.company='Westcon-Comstor';pptx.defineSlideMaster({title:'MASTER',background:{color:'FFFFFF'},objects:[{rect:{x:0,y:0,w:13.333,h:.18,fill:{color:'F09E0D'},line:{color:'F09E0D'}}},{text:{text:'WESTCON IBERIA · EVIDENCE-TO-ACTION',options:{x:.55,y:.25,w:6,h:.25,fontFace:'Corbel',fontSize:9,bold:true,color:'3195BB'}}},{text:{text:'Motor v9 · Inteligencia pública',options:{x:9.7,y:.25,w:3.0,h:.25,fontFace:'Corbel',fontSize:8,color:'687B8D',align:'right'}}}],slideNumber:{x:12.75,y:7.12,color:'687B8D',fontSize:8}});const addTitle=(s,t,sub='')=>{s.addText(t,{x:.55,y:.65,w:12.2,h:.55,fontFace:'Corbel',fontSize:27,bold:true,color:'082335',margin:0});if(sub)s.addText(sub,{x:.55,y:1.24,w:11.9,h:.4,fontFace:'Corbel',fontSize:11,color:'687B8D',margin:0})};const top=[...state.vendors].sort(verifiedDecisionSort).slice(0,14);let s;
  if(moduleOn('executive')){s=pptx.addSlide('MASTER');addTitle(s,'Decidir rápido. Ejecutar con evidencia.','Cada decisión conecta realidad pública, responsable, hitos 30/60/90, KPI y condición go/no-go.');const k=[[state.vendors.length,'fabricantes'],[allEvidence().length,'evidencias'],[Object.keys(state.reality?.vendors||{}).length,'contratos verificados'],[state.decision.roles.length,'perfiles Westcon']];k.forEach((x,i)=>{s.addShape(pptx.ShapeType.rect,{x:.55+i*3.05,y:2,w:2.8,h:1.15,fill:{color:i%2?'F7F9FA':'F2F7F8'},line:{color:'DBE4E9'}});s.addText(String(x[0]),{x:.75+i*3.05,y:2.2,w:2.3,h:.42,fontFace:'Corbel',fontSize:24,bold:true,color:'082335'});s.addText(x[1],{x:.75+i*3.05,y:2.68,w:2.3,h:.24,fontFace:'Corbel',fontSize:9,color:'687B8D'})});s.addText(top.slice(0,4).map(v=>`${v.name} · ${v.analysis.recommendation} · ${v.derived.plan.owner}\n90 días: ${v.derived.plan.p90}`).join('\n\n'),{x:.65,y:3.45,w:11.7,h:3.0,fontFace:'Corbel',fontSize:11,bold:true,color:'113A50',breakLine:true,margin:0})}
  ...
  if(moduleOn('methodology')){s=pptx.addSlide('MASTER');addTitle(s,'Metodología','Simplicidad visible; evidencia y ejecución trazables.');s.addText('14 dimensiones estratégicas\n+ riesgo y estabilidad\n+ matriz fabricante × capacidad\n+ realidad + implicación\n+ plan 30 / 60 / 90\n+ KPI + go/no-go\n+ fuentes públicas',{x:.65,y:1.8,w:4.0,h:3.7,fontFace:'Corbel',fontSize:19,bold:true,color:'082335',margin:0,breakLine:true});s.addText('El motor v9 no confunde score con recomendación. Los contratos verificados conectan hechos públicos con una ejecución concreta; el resto queda en INVESTIGAR y muestra el dato faltante. El gate interno se conserva solo en el navegador, permanece separado de la evidencia pública y requiere revisión humana.',{x:5.0,y:1.8,w:7.3,h:3.3,fontFace:'Corbel',fontSize:14,color:'445D6C',margin:0})}
  await pptx.writeFile({fileName:'Westcon_Iberia_Evidence_to_Action_v2.0.pptx'});toast('PowerPoint generado')
}
