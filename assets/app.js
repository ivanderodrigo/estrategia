...
const state={data:null,base:null,research:null,reality:null,engine:null,decision:null,capability:null,ecosystem:null,status:null,changes:null,vendors:[],selected:null,selectedRole:'country_manager',quick:'all',deep:false,fontScale:1,reportModules:new Set()};
...
async function load(){
  const [data,base,research,reality,engine,decision,capability,ecosystem,status,changes]=await Promise.all([
  ...
  state.data=data;state.base=base;state.research=research;state.reality=reality;state.engine=engine;state.decision=decision;state.capability=capability;state.ecosystem=ecosystem;state.status=status;state.changes=changes;state.reportModules=new Set((decision.exportModules||[]).filter(x=>x.default).map(x=>x.id));
  state.vendors=data.vendors.map(enrichVendor);
  initNav();renderAll();
}

function allEvidence(){
  const rows=[...(state.research?.evidence||[]),...(state.reality?.facts||[])],map=new Map();
...
function enrichVendor(v){
  ...
  const baseRecommendation=recommendationFor(analysis),stability=decisionStability(v,analysis,baseRecommendation),reality=realityFor(v);analysis.decisionStability=stability.score;analysis.recommendationAlternatives=stability.alternatives;analysis.baseRecommendation=baseRecommendation;analysis.computedRecommendation=robustRecommendation(baseRecommendation,analysis,stability.score);analysis.recommendation=reality?.decision||analysis.computedRecommendation;analysis.recommendationSource=reality?'VERIFIED_MARKET_REALITY':'MODEL_WITH_EVIDENCE_GATE';analysis.recommendationConfidence=reality?.confidence||reliability;
...
function renderDataHealth(){
  const items=[['Contratos verificables',state.vendors.filter(v=>realityFor(v)).length,state.vendors.length,'Realidad + 30/60/90 + owner + KPI + go/no-go'],['Canal ES/PT',state.vendors.filter(v=>v.derived.channels.length).length,state.vendors.length,'Mayoristas alternativos identificados'],['Integradores',state.vendors.filter(v=>v.derived.integrators.length).length,state.vendors.length,'Partners/integradores con prueba pública'],['Clientes públicos',state.vendors.filter(v=>v.derived.customers.length).length,state.vendors.length,'Referencias finales ES/PT'],['Demanda pública',state.vendors.filter(v=>v.analysis.publicDemand>=35).length,state.vendors.length,'Señales estructuradas de contratación ES/PT'],['Consultoras',state.vendors.filter(v=>v.analysis.analystSignal>=50).length,state.vendors.length,'Señal pública específica o de mercado'],['Evidencia fuerte',state.vendors.filter(v=>v.analysis.evidenceConfidence>=65).length,state.vendors.length,'Cobertura suficiente para lectura ejecutiva']];
...
function realityContractHtml(v){
  ...
  if(!r){const ready=gateReadiness(v);return `<section class="reality-contract blocked"><div class="contract-head"><span>DECISIÓN BLOQUEADA</span><b>INVESTIGAR · evidencia insuficiente para una apuesta comercial</b></div><p>No se presenta una acción como realidad de mercado hasta demostrar pipeline, partner/ruta de compra, caso de uso, capacidad operativa y economía. Gate interno actual: ${ready.answered}/${ready.total}.</p><div class="contract-gate"><b>Condición de salida</b><span>${esc(p.goNoGo)}</span></div></section>`}
...
function selectVendor(name){
  ...
  <div class="plan-block"><h4>PLAN PROPUESTO</h4><div><b>30 días</b><p>${esc(v.derived.plan.p30)}</p></div><div><b>60 días</b><p>${esc(v.derived.plan.p60)}</p></div><div><b>90 días</b><p>${esc(v.derived.plan.p90)}</p></div><div><b>Gate</b><p>${esc(v.derived.plan.p180)}</p></div></div>
...
function renderEngine(){
  ...
  const w=state.engine.opportunityWeights;$('#engineExplanation').innerHTML=`<p><b>Motor v9 Evidence-to-Action.</b> Mantiene el scoring multidimensional y añade un contrato estricto: realidad observada, implicación, acción 30/60/90, responsable, KPI, go/no-go y fuentes. Las hipótesis sin prueba específica quedan en <b>INVESTIGAR</b>; no se disfrazan de recomendación. Después puntúa <b>${state.decision.actions.length} acciones</b> para <b>${state.decision.roles.length} perfiles</b> y valida la aplicabilidad de cada capacidad Westcon.</p><div class="weight-list">${Object.entries(w).map(([k,v])=>`<div><span>${esc(labels[k]||k)}</span><b>${Math.round(v*100)}%</b><i style="--w:${v*100}%"></i></div>`).join('')}</div><p class="tiny">La evidencia pública y los datos internos permanecen separados. El gate interno se guarda localmente, no sube la puntuación pública y nunca autoriza inversión de forma automática.</p>`
}
