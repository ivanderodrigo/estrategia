...
const state={data:null,base:null,research:null,reality:null,graph:null,engine:null,decision:null,capability:null,ecosystem:null,universe:null,schedule:null,discoveredEntities:null,sourceHealth:null,researchErrors:null,runManifest:null,supervisor:null,status:null,changes:null,v34Recommendations:null,v34Entities:null,v34Relationships:null,v34Brief:null,v34Architectures:null,v34History:null,v34SourceCoverage:null,v34Quality:null,v34TableConfig:null,v34EcosystemMotion:null,vendors:[],selected:null,selectedRole:'country_manager',quick:'all',deep:false,fontScale:1,reportModules:new Set(),vendorColumns:[],vendorSort:'decisionScore',vendorSortDirection:'desc'};
...
async function load(){
  const [data,base,research,reality,graph,engine,decision,capability,ecosystem,universe,schedule,discoveredEntities,sourceHealth,researchErrors,runManifest,supervisor,status,changes,v34Recommendations,v34Entities,v34Relationships,v34Brief,v34Architectures,v34History,v34SourceCoverage,v34Quality,v34TableConfig,v34EcosystemMotion]=await Promise.all([
    ...
    fetch('data/market_reality.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/intelligence_graph.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    ...
    fetch('config/source_universe.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('config/update_schedule.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/discovered_entities.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/source_health.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/research_errors.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/run_manifest.latest.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/supervisor.latest.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/research_status.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/changes.latest.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/recommendations.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/entities.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/relationships.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/business_intelligence_report.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/architectures.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/historical_intelligence.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/source_coverage.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/quality_report.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('config/v34/table_config.json').then(r=>r.ok?r.json():{}).catch(()=>({})),
    fetch('data/v34/ecosystem_motion_intelligence.json').then(r=>r.ok?r.json():{}).catch(()=>({}))
  ]);
  state.data=data;state.base=base;state.research=research;state.reality=reality;state.graph=graph;state.engine=engine;state.decision=decision;state.capability=capability;state.ecosystem=ecosystem;state.universe=universe;state.schedule=schedule;state.discoveredEntities=discoveredEntities;state.sourceHealth=sourceHealth;state.researchErrors=researchErrors;state.runManifest=runManifest;state.supervisor=supervisor;state.status=status;state.changes=changes;state.v34Recommendations=v34Recommendations;state.v34Entities=v34Entities;state.v34Relationships=v34Relationships;state.v34Brief=v34Brief;state.v34Architectures=v34Architectures;state.v34History=v34History;state.v34SourceCoverage=v34SourceCoverage;state.v34Quality=v34Quality;state.v34TableConfig=v34TableConfig;state.v34EcosystemMotion=v34EcosystemMotion;state.reportModules=new Set((decision.exportModules||[]).filter(x=>x.default).map(x=>x.id));
  state.vendors=data.vendors.map(enrichVendor);
  initNav();renderAll();window.WestconV340?.render(state);
}

function allEvidence(){
  const graphClaims=(state.graph?.claims||[]).map(c=>{const s=(state.graph?.sources||[]).find(x=>x.id===c.sourceIds?.[0])||{};return {...c,title:`${c.entity} · ${c.dimension}`,summary:c.summary,url:s.url,source:s.publisher,sourceTier:s.tier,date:s.date,scope:c.scope,evidenceType:c.dimension}});
  const rows=[...(state.research?.evidence||[]),...(state.reality?.facts||[]),...graphClaims],map=new Map();
  rows.forEach(e=>map.set(e.id||e.url||`${e.title}|${e.source}`,e));return [...map.values()];
}
function sourceById(id){return (state.graph?.sources||[]).find(x=>x.id===id)}
function claimById(id){return (state.graph?.claims||[]).find(x=>x.id===id)}
function confidenceBand(n){return (state.graph?.policy?.confidenceBands||[]).find(x=>Number(n)>=Number(x.min))||{label:'Descubrimiento',meaning:'Pista para investigar.'}}
function intel(value,ids=[],label='Dato público',confidenceOverride=null){
  const claims=(ids||[]).map(claimById).filter(Boolean),directSources=(ids||[]).map(sourceById).filter(Boolean),sources=[...new Map([...directSources,...uniq(claims.flatMap(c=>c.sourceIds||[])).map(sourceById).filter(Boolean)].map(x=>[x.id,x])).values()],conf=confidenceOverride??(claims.length?Math.min(...claims.map(c=>Number(c.confidence||0))):sources.length?Math.min(...sources.map(s=>Number(s.confidence||0))):0),band=confidenceBand(conf);
  const tips=claims.map(c=>`${c.scope}: ${c.summary}`).join(' · '),links=sources.map(s=>`<a href="${esc(s.url)}" target="_blank" rel="noopener">${esc(s.publisher)} · ${fmtDate(s.date)} ↗</a>`).join('');
  return `<span class="intel" tabindex="0">${esc(value)}<span class="intel-mark">i</span><span class="intel-tip"><b>${esc(label)}</b><strong>${conf}/100 · ${esc(band.label)}</strong><small>${esc(tips||'Sin evidencia asociada; no usar para decisión.')}</small><em>${esc(band.meaning)}</em>${links}</span></span>`
}
function derivedIntel(value,label,v,note='Inferencia propia recalculada'){
  const ev=(v?.derived?.evidence||[]).slice().sort((a,b)=>Number(b.confidence||0)-Number(a.confidence||0)).slice(0,3),conf=v?.analysis?.evidenceConfidence||0,links=ev.map(e=>e.url?`<a href="${esc(e.url)}" target="_blank" rel="noopener">${esc(e.source||e.sourceTier)} · ${fmtDate(e.date||e.published)} ↗</a>`:'').join('');
  return `<span class="derived-intel" tabindex="0">${value}<span class="intel-mark">i</span><span class="intel-tip"><b>${esc(label)}</b><strong>${clamp(conf)}/100 · confianza de evidencia</strong><small>${esc(note)}. Ámbito ${esc((v?.countries||[]).join(' + ')||'no definido')}.</small><em>Umbral ejecutivo: una puntuación nunca autoriza por sí sola una acción.</em>${links}</span></span>`
}
function realityFor(v){return state.reality?.vendors?.[typeof v==='string'?v:v?.name]||null}
function v34RecommendationFor(v){const name=norm(typeof v==='string'?v:v?.name),rank={'ACTUAR':4,'PREPARAR / VALIDAR':3,'INVESTIGAR':2,'VIGILAR':1};return (state.v34Recommendations?.recommendations||[]).filter(r=>[...(r.vendors_involved||[]),...(r.integrators_involved||[]),...(r.distributors_involved||[])].some(x=>norm(x)===name)).sort((a,b)=>(b.impact_potential?.score||0)-(a.impact_potential?.score||0)||(rank[b.action_type]||0)-(rank[a.action_type]||0))[0]||null}
function realityEvidence(v){const ids=new Set(realityFor(v)?.evidenceIds||[]);return (state.reality?.facts||[]).filter(e=>ids.has(e.id))}
function relevantEvidence(v){
  const name=norm(v.name);
  return allEvidence().filter(e=>{
    if((e.vendors||[]).some(x=>norm(x)===name))return true;
    if((realityFor(v)?.evidenceIds||[]).includes(e.id))return true;
...
function kpiFor(action,v){
  if(action.kpis?.length)return action.kpis.join(' · ');
  const ra=realityFor(v)?.action;if(ra?.kpis?.length&&action.id==='market-contract')return ra.kpis.join(' · ');
...
function roleMandate(v,roleId){
  const r=realityFor(v),a=r?.action;if(!a)return '';
  const partners=(r.targetIntegrators||[]).slice(0,4).join(', ')||'partners por validar';
  const map={
    country_manager:`Autorizar ${a.objective} Primer gate: ${a.day30}`,
    director_vsm_sa:`Orquestar fabricante y preventa para ejecutar: ${a.day30} Asegurar después: ${a.day60}`,
    director_psm:`Asignar responsables sobre ${partners}. ${a.day60}`,
    vsm:`Convertir la tesis en plan de fabricante: ${a.day30}`,
    psm:`Activar y cualificar a ${partners}. ${a.day60}`,
    sa:`Diseñar y demostrar el caso de uso: ${a.day60}`,
    marketing:`No lanzar campaña amplia. Construir una audiencia verificable desde este gate: ${a.day30}`,
    services:`Definir alcance, SLA, margen y attach de servicios dentro de: ${a.day60}`,
    operations:`Instrumentar el seguimiento y llegar a este resultado: ${a.day90}`,
    finance:`Liberar inversión por tramos contra KPI y aplicar esta salida: ${a.goNoGo}`,
    logistics:`Validar BOM, disponibilidad y lead time solo para las oportunidades creadas por: ${a.day60}`
  };return map[roleId]||`${a.objective} ${a.day30}`;
}
function actionPlainLanguage(v,action,roleId){
  ...
  if(action.id==='market-contract')return roleMandate(v,roleId);
...
function recEvidence(v,action){
  if(action.id==='market-contract')return realityEvidence(v).slice(0,5);
...
function roleRecommendations(v,a,t){
  const out={};(state.decision.roles||[]).forEach(role=>{
    const reality=realityFor(v),contract=reality?{id:'market-contract',name:`Mandato verificado · ${reality.decision}`,category:'Realidad de mercado',roles:[role.id],score:reality.confidence,value:reality.thesis,basis:[{metric:'evidenceConfidence',value:reality.confidence}],evidence:realityEvidence(v),kpis:reality.action?.kpis||[],goNoGo:reality.action?.goNoGo}:null;
    let rows=(state.decision.actions||[]).filter(x=>(x.roles||[]).includes(role.id)).filter(x=>capabilityStatus(v,x.capabilityId).eligible).map(x=>{const sc=actionScore(x,v,a,t);const row={...x,score:sc.score,basis:sc.basis};return {...row,evidence:recEvidence(v,row)};}).filter(x=>x.score>=44&&x.evidence.length).sort((x,y)=>y.score-x.score);
    const picked=contract?[contract]:[],cats=new Set(contract?['Realidad de mercado']:[]);for(const x of rows){if(picked.length>=3)break;if(cats.has(x.category)&&picked.length<2)continue;cats.add(x.category);picked.push(x);}
    if(!picked.length){const best=rows[0],missing=[];if(!v.derived?.integrators?.length)missing.push('integrador Iberia');if(!v.derived?.customers?.length)missing.push('referencia de cliente');if(!v.derived?.channels?.length)missing.push('ruta de canal');if(a.evidenceConfidence<55)missing.push('evidencia reciente y específica');out[role.id]=[{id:'no-action',name:'VALIDAR ANTES DE INVERTIR',category:'Gate de evidencia',roles:[role.id],score:best?.score||35,value:`No autorizar una acción de mercado para ${v.name}. Falta demostrar: ${missing.join(', ')||'pipeline, partner, caso de uso y economía de la acción'}.`,basis:best?.basis||[],evidence:[],kpis:['5 datos internos completos','1 fuente pública específica','responsable y fecha de decisión']}];}
...
function executivePlan(v,a,archetype,recs,legacy){
  const reality=realityFor(v);
  if(reality?.action){const x=reality.action;return {...legacy,p30:x.day30,p60:x.day60,p90:x.day90,p180:x.goNoGo,executiveActions:executiveActions(recs),archetype,reality,thesis:reality.thesis,owner:x.owner,kpis:x.kpis||[],goNoGo:x.goNoGo,targetIntegrators:reality.targetIntegrators||[],channelCompetitors:reality.channelCompetitors||[]};}
  const all=executiveActions(recs),roleName=id=>(state.decision.roles||[]).find(r=>r.id===id)?.name||id;
  return {...legacy,p60:legacy.p90,executiveActions:all,archetype,thesis:`${archetype.name}. ${archetype.description}`,owner:'Country Manager + VSM',kpis:['gate de evidencia completo','pipeline cualificado','conversión por acción'],goNoGo:'No escalar hasta demostrar partner, cuenta/caso de uso, capacidad operativa, economía y una fuente pública específica.'};
}
...
function roleRecommendationHtml(v,roleId){
  ...
  return `<div class="role-head"><b>${esc(role?.name||roleId)}</b><span>${esc(role?.scope||'')}</span></div>${rows.map(x=>{const cs=capabilitySourceFor(x),plain=actionPlainLanguage(v,x,roleId),cap=capabilityStatus(v,x.capabilityId);return `<div class="role-rec ${x.id==='market-contract'?'verified-rec':''}"><div><strong>${x.score}</strong><b>${esc(x.name)}</b></div>${x.id!=='no-action'&&x.id!=='market-contract'?capabilityBadge(v,x):''}<p><b>Mandato.</b> ${esc(plain)}</p><p class="rec-why"><b>Realidad detectada.</b> ${esc(x.value)}</p><div class="rec-basis">${x.basis.map(b=>`<span>${esc(decisionMetricLabel(b.metric))} <b>${b.value}</b></span>`).join('')}</div><p class="rec-kpi"><b>Resultado / KPI.</b> ${esc(kpiFor(x,v))}</p>${x.goNoGo?`<p class="rec-gate"><b>Gate.</b> ${esc(x.goNoGo)}</p>`:''}${x.id!=='no-action'&&x.capabilityId?`<small class="method-note">Compatibilidad: ${esc(cap.status.replaceAll('_',' '))} · ${esc(cap.scope||'ámbito por validar')}. ${esc(cap.reason||'')}</small>`:''}${x.id!=='no-action'&&x.id!=='market-contract'&&cs?`<a class="evidence-link" href="${cs.url}" target="_blank" rel="noopener">Capacidad Westcon · ${esc(cap.programme?.name||cs.name)} ↗</a>`:''}${x.evidence?.length?`<details><summary>${x.evidence.length} fuentes que soportan la acción</summary>${x.evidence.map(e=>`<a class="evidence-link" href="${e.url}" target="_blank" rel="noopener">${esc(e.source||e.sourceTier)} · ${esc(e.title)} ↗</a>`).join('')}</details>`:`<small class="method-note">Sin evidencia pública específica: la recomendación queda bloqueada en validación.</small>`}</div>`}).join('')||'<p>Sin recomendación suficientemente fuerte para este perfil.</p>'}`;
}
...
function enrichVendor(v){
  ...
  const baseRecommendation=recommendationFor(analysis),stability=decisionStability(v,analysis,baseRecommendation),reality=realityFor(v),v34rec=v34RecommendationFor(v);analysis.decisionStability=stability.score;analysis.recommendationAlternatives=stability.alternatives;analysis.baseRecommendation=baseRecommendation;analysis.computedRecommendation=robustRecommendation(baseRecommendation,analysis,stability.score);analysis.v34Recommendation=v34rec;analysis.actionAuthorized=v34rec?.action_type==='ACTUAR';analysis.recommendation=v34rec?.action_type||'INVESTIGAR';analysis.recommendationSource=v34rec?'V34_EVIDENCE_PROPORTIONAL':'V34_RESEARCH_DEFAULT';analysis.recommendationConfidence=Math.round(100*Number(v34rec?.confidence?.score||reality?.confidence/100||reliability/100));
...
function initNav(){
  $$('#tabs button').forEach(b=>b.onclick=()=>{switchView(b.dataset.view);$('#tabs').classList.remove('open');$('#navToggle')?.setAttribute('aria-expanded','false')});$$('[data-jump]').forEach(b=>b.onclick=()=>switchView(b.dataset.jump));
  $('#navToggle')?.addEventListener('click',()=>{const open=$('#tabs').classList.toggle('open');$('#navToggle').setAttribute('aria-expanded',String(open))});
  ...
  $('#distributorSearch')?.addEventListener('input',renderDistributors);$('#distributorCountry')?.addEventListener('change',renderDistributors);$('#integratorSearch')?.addEventListener('input',renderIntegrators);$('#integratorCountry')?.addEventListener('change',renderIntegrators);
  $('#columnToggle')?.addEventListener('click',()=>{const open=$('#columnPicker').classList.toggle('open');$('#columnToggle').setAttribute('aria-expanded',String(open))});
  $('#vendorSort')?.addEventListener('change',e=>{state.vendorSort=e.target.value;localStorage.setItem('westcon-v3-sort',state.vendorSort);renderVendorTable()});
  $('#vendorSortDirection')?.addEventListener('click',()=>{state.vendorSortDirection=state.vendorSortDirection==='desc'?'asc':'desc';localStorage.setItem('westcon-v3-sort-dir',state.vendorSortDirection);renderTableStudio();renderVendorTable()});
...
function renderAll(){state.fontScale=Number(localStorage.getItem('westcon-font-scale')||1);document.documentElement.style.setProperty('--font-scale',state.fontScale);initVendorColumns();renderKpis();renderDecisionCards();renderChanges();renderDataHealth();renderFilters();renderTableStudio();renderOverviewVendors();renderVendorTable();renderDistributors();renderIntegrators();renderArchitectures();renderTrends();renderSignals();renderDataKpis();renderEngine();renderGaps();renderSources();renderResearch();renderOperations()}
function verifiedDecisionSort(a,b){return Number(Boolean(realityFor(b)))-Number(Boolean(realityFor(a)))||b.analysis.recommendationConfidence-a.analysis.recommendationConfidence||b.analysis.decisionScore-a.analysis.decisionScore||a.name.localeCompare(b.name)}
function renderKpis(){
  const ev=allEvidence(),ints=state.vendors.reduce((s,v)=>s+v.derived.integrators.length,0),cust=state.vendors.reduce((s,v)=>s+v.derived.customers.length,0),contracts=Object.keys(state.reality?.vendors||{}).length;
  const k=[[state.vendors.length,'Fabricantes','Portfolio FY27'],[ev.length,'Evidencias','públicas trazables'],[ints,'Integradores','señales Iberia'],[cust,'Clientes públicos','referencias ES/PT'],[contracts,'Contratos','decisión + owner + KPI + gate']];
  $('#marketKpis').innerHTML=k.map(x=>`<div class="kpi"><span>${x[1]}</span><strong>${x[0]}</strong><small>${x[2]}</small></div>`).join('')
}
function renderDecisionCards(){
  const rows=(state.graph?.claims||[]).filter(c=>c.confidence>=94).slice(-8).reverse().slice(0,4);
  $('#decisionCards').innerHTML=rows.map(c=>`<article class="decision-card"><div class="decision-head"><span class="priority OPTIMIZAR">HECHO PÚBLICO</span><b>${intel(c.confidence,[c.id],'Confianza')}</b></div><h3>${esc(c.entity)}</h3><p>${esc(c.summary)}</p><div class="mini-metrics">${intel(c.value,[c.id],c.dimension)}<span class="metric-pill confidence"><small>${esc(c.scope)}</small></span></div></article>`).join('');
}
function entityClaimHtml(ids){return (ids||[]).map(id=>{const c=claimById(id);return c?`<div class="entity-claim"><span>${esc(c.dimension)}</span><b>${intel(c.value,[c.id],c.dimension)}</b><small>${esc(c.summary)}</small></div>`:''}).join('')}
function entityUniverse(kind){const map=new Map();(state.universe?.[kind]||[]).forEach(x=>map.set(norm(x.name),{...x,origin:'seed'}));(state.discoveredEntities?.[kind]||[]).forEach(x=>{const key=norm(x.name),current=map.get(key);map.set(key,current?{...x,...current,countries:uniq([...(current.countries||[]),...(x.countries||[])]),dynamicStatus:x.status}:{...x,origin:'dynamic',type:`${kind==='distributors'?'mayorista':'integrador'} descubierto`,dynamicStatus:x.status})});return [...map.values()]}
function renderDistributors(){
  const root=$('#distributorCards');if(!root)return;const q=norm($('#distributorSearch')?.value),country=$('#distributorCountry')?.value||'all',graph=state.graph?.distributors||[],universe=entityUniverse('distributors'),rows=universe.map(seed=>{const g=graph.find(x=>norm(x.name)===norm(seed.name))||{},signals=(state.research?.channelSignals||[]).filter(x=>norm(x.distributor)===norm(seed.name)),vendors=uniq(signals.map(x=>x.vendor)),claims=g.scaleClaimIds||[],proof=claims.length+signals.filter(x=>Number(x.confidence||0)>=70).length;return {...seed,...g,countries:g.countries||seed.countries||[],scaleClaimIds:claims,signals,vendors,proof}}).filter(d=>(country==='all'||d.countries.includes(country))&&(!q||norm([d.name,d.type,d.positioning,...d.countries,...d.vendors].join(' ')).includes(q))).sort((a,b)=>b.proof-a.proof||a.name.localeCompare(b.name));
  $('#distributorCount').textContent=`${rows.length} de ${universe.length} seguidos`;root.innerHTML=rows.map(d=>{const status=d.scaleClaimIds.length?'verified':d.signals.length?'partial':'';return `<article class="entity-card"><div class="entity-head"><div><span>${esc(d.countries.join(' + ')||'Ámbito por verificar')}</span><h3>${esc(d.name)}</h3></div><span class="entity-status ${status}">${status==='verified'?'DATOS VERIFICADOS':status==='partial'?'SEÑALES EN VALIDACIÓN':'EN INVESTIGACIÓN'}</span></div><div class="entity-meta"><span>${esc(d.type||'tipología pendiente')}</span><span>${d.signals.length} relaciones detectadas</span><span>${d.vendors.length} fabricantes asociados</span></div><p>${esc(d.positioning||'Perfil abierto: el motor está reuniendo linecard ES/PT, escala pública, servicios y diferenciación.')}</p><div class="claim-grid">${entityClaimHtml(d.scaleClaimIds)}</div><h4>Fabricantes públicamente asociados</h4><div>${tags(d.vendors,'',10)}</div>${d.overlap?`<h4>Solape con Westcon</h4><p>${esc(d.overlap)}</p>`:''}<h4>Ventajas demostrables / declaradas</h4><div>${tags(d.advantages||[])}</div><div class="gap-note"><b>Pendiente de corroborar</b>${esc((d.gaps||['linecard ES/PT vigente','cifras públicas comparables','servicios y certificaciones','relaciones terminadas']).join(' · '))}</div></article>`}).join('')
}
function renderIntegrators(){
  const root=$('#integratorCards');if(!root)return;const q=norm($('#integratorSearch')?.value),country=$('#integratorCountry')?.value||'all',graph=state.graph?.integrators||[],universe=entityUniverse('integrators'),rows=universe.map(seed=>{const g=graph.find(x=>norm(x.name)===norm(seed.name))||{},signals=(state.research?.integratorSignals||[]).filter(x=>norm(x.name)===norm(seed.name)),customers=(state.research?.customerSignals||[]).filter(x=>norm(x.integrator)===norm(seed.name)),vendors=uniq([...(g.vendors||[]),...signals.map(x=>x.vendor)]),claims=g.claimIds||[],proof=claims.length+signals.filter(x=>Number(x.confidence||0)>=70).length+customers.length;return {...seed,...g,countries:g.countries||seed.countries||[],claimIds:claims,signals,customers,vendors,proof}}).filter(d=>(country==='all'||d.countries.includes(country))&&(!q||norm([d.name,d.type,...d.countries,...(d.specializations||[]),...d.vendors].join(' ')).includes(q))).sort((a,b)=>b.proof-a.proof||a.name.localeCompare(b.name));
  $('#integratorCount').textContent=`${rows.length} de ${universe.length} seguidos`;root.innerHTML=rows.map(d=>{const status=d.claimIds.length?'verified':d.signals.length||d.customers.length?'partial':'';return `<article class="entity-card"><div class="entity-head"><div><span>${esc(d.countries.join(' + ')||'Ámbito por verificar')}</span><h3>${esc(d.name)}</h3></div><span class="entity-status ${status}">${status==='verified'?'PRUEBA PÚBLICA':status==='partial'?'SEÑALES EN VALIDACIÓN':'EN INVESTIGACIÓN'}</span></div><div class="entity-meta"><span>${esc(d.type||'integrador')}</span><span>${d.signals.length} relaciones</span><span>${d.customers.length} clientes públicos</span></div><div>${tags(d.specializations||[],'synergy')}</div><div class="claim-grid">${entityClaimHtml(d.claimIds)}</div><h4>Portfolio públicamente asociado</h4><div>${tags(d.vendors,'',12)}</div><div class="gap-note"><b>Pendiente</b>${esc((d.gaps||['certificaciones por fabricante y nivel','clientes ES/PT','volumen público','distribuidores utilizados','especializaciones vigentes']).join(' · '))}</div></article>`}).join('')
}
function renderArchitectures(){const root=$('#architectureCards');if(!root)return;root.innerHTML=(state.graph?.architectures||[]).map(a=>`<article class="architecture-card ${a.classification.toLowerCase()}"><div class="architecture-head"><span class="arch ${a.classification==='COMPLEMENTARY'?'complementary':a.classification==='CONDITIONAL'?'conditional':'conflict'}">${a.classification==='COMPLEMENTARY'?'COMPLEMENTARIA':a.classification==='CONDITIONAL'?'CONDICIONAL':'CONFLICTO'}</span>${intel(a.confidence,a.sourceIds,'Confianza técnica',a.confidence)}</div><h3>${esc(a.name)}</h3><div>${tags(a.vendors,a.classification==='CONFLICT'?'overlap':'synergy',6)}</div><p>${esc(a.rationale)}</p><div class="architecture-boundary"><b>Frontera obligatoria</b>${esc(a.boundary)}</div><small>Capas: ${esc(a.layers.join(' → '))}</small></article>`).join('')}
...
function renderDataHealth(){
  const recs=state.v34Recommendations?.recommendations||[],items=[['Recomendaciones trazables',recs.filter(r=>r.evidence?.length).length,recs.length||1,'Con evidencia enlazada y auditada'],['Canal ES/PT',state.vendors.filter(v=>v.derived.channels.length).length,state.vendors.length,'Mayoristas alternativos identificados'],['Integradores',state.vendors.filter(v=>v.derived.integrators.length).length,state.vendors.length,'Partners/integradores con prueba pública'],['Clientes públicos',state.vendors.filter(v=>v.derived.customers.length).length,state.vendors.length,'Referencias finales ES/PT'],['Demanda pública',state.vendors.filter(v=>v.analysis.publicDemand>=35).length,state.vendors.length,'Señales estructuradas de contratación ES/PT'],['Consultoras',state.vendors.filter(v=>v.analysis.analystSignal>=50).length,state.vendors.length,'Señal pública específica o de mercado'],['Evidencia fuerte',state.vendors.filter(v=>v.analysis.evidenceConfidence>=65).length,state.vendors.length,'Cobertura suficiente para lectura ejecutiva']];
...
function renderFilters(){const domains=uniq(state.vendors.map(v=>v.domain)).sort();$('#domainFilter').innerHTML='<option value="all">Todas las áreas</option>'+domains.map(d=>`<option>${esc(d)}</option>`).join('');const rec=['ACTUAR','PREPARAR / VALIDAR','INVESTIGAR','VIGILAR'];$('#recommendationFilter').innerHTML='<option value="all">Todos los tipos de acción</option>'+rec.map(x=>`<option>${x}</option>`).join('')}
...
const VENDOR_COLUMNS={
  vendor:{label:'Fabricante',sort:v=>v.name,text:v=>`${v.name} · ${v.domain}`,html:v=>`<span class="vendor-name">${esc(v.name)}</span><span class="tiny">${v.countries.join(' · ')} · ${esc(v.domain)}</span>`},
  decision:{label:'Tipo de acción',sort:v=>v.analysis.recommendationConfidence,text:v=>v.analysis.recommendation,html:v=>derivedIntel(`<span class="priority ${v.analysis.recommendation}">${v.analysis.recommendation}</span>`,'Acción proporcional',v,v.analysis.v34Recommendation?'Hecho, interpretación y riesgo evaluados por separado':'Hipótesis material pendiente de investigación')},
  opportunity:{label:'Oportunidad',sort:v=>v.analysis.opportunity,text:v=>String(v.analysis.opportunity),html:v=>scoreCell(v,'opportunity')},
  marketMomentum:{label:'Mercado',sort:v=>v.analysis.marketMomentum,text:v=>String(v.analysis.marketMomentum),html:v=>scoreCell(v,'marketMomentum','good')},
  vendorMomentum:{label:'Vendor',sort:v=>v.analysis.vendorMomentum,text:v=>String(v.analysis.vendorMomentum),html:v=>scoreCell(v,'vendorMomentum','good')},
  competitors:{label:'Competidores',sort:v=>(v.marketCompetitors||[]).length,text:v=>(v.marketCompetitors||[]).join(', ')||'Sin dato',html:v=>tags(v.marketCompetitors,'',3)},
  channel:{label:'Mayoristas ES / PT',sort:v=>v.analysis.channelPressure,text:v=>v.derived.channels.map(x=>`${x.country}:${x.distributor}`).join(' · ')||'Por demostrar',html:v=>channelSummary(v)},
  ecosystem:{label:'Ecosistema',sort:v=>v.analysis.ecosystemStrength,text:v=>`${v.derived.integrators.length} integradores · ${v.derived.customers.length} clientes`,html:v=>ecosystemSummary(v)},
  analyst:{label:'Consultoras',sort:v=>v.analysis.analystSignal,text:v=>(v.analystSignals||[]).map(x=>x.analyst).join(', ')||'Sin señal',html:v=>analystSummary(v)},
  synergy:{label:'Sinergia',sort:v=>v.analysis.synergyPotential,text:v=>String(v.analysis.synergyPotential),html:v=>scoreCell(v,'synergyPotential','good')},
  overlap:{label:'Solape',sort:v=>v.analysis.overlapRisk,text:v=>String(v.analysis.overlapRisk),html:v=>scoreCell(v,'overlapRisk','bad')},
  confidence:{label:'Confianza',sort:v=>v.analysis.evidenceConfidence,text:v=>String(v.analysis.evidenceConfidence),html:v=>derivedIntel(`<span class="confidence-dot ${v.analysis.evidenceConfidence>=65?'high':v.analysis.evidenceConfidence>=45?'mid':'low'}"></span>${v.analysis.evidenceConfidence}<span class="tiny">${evidenceLabel(v)}</span>`,'Confianza de evidencia',v,'Autoridad, frescura, geografía, directitud, corroboración, diversidad y especificidad')},
  recurring:{label:'Recurrencia',sort:v=>v.analysis.recurringPotential,text:v=>String(v.analysis.recurringPotential),html:v=>scoreCell(v,'recurringPotential','good')},
  publicDemand:{label:'Demanda pública',sort:v=>v.analysis.publicDemand,text:v=>String(v.analysis.publicDemand),html:v=>scoreCell(v,'publicDemand')},
  risk:{label:'Riesgo',sort:v=>v.analysis.risk,text:v=>String(v.analysis.risk),html:v=>scoreCell(v,'risk','bad')},
  completeness:{label:'Completitud',sort:v=>v.analysis.dataCompleteness,text:v=>String(v.analysis.dataCompleteness),html:v=>scoreCell(v,'dataCompleteness')},
  whiteSpace:{label:'White space',sort:v=>v.analysis.whiteSpace,text:v=>String(v.analysis.whiteSpace),html:v=>scoreCell(v,'whiteSpace','good')},
  gate:{label:'Siguiente paso',sort:v=>v.analysis.v34Recommendation?.impact_potential?.score||0,text:v=>v.analysis.v34Recommendation?.action||'Obtener la evidencia prioritaria que falta.',html:v=>`<span class="tiny">${esc(v.analysis.v34Recommendation?.action||'Investigar la señal y documentar la evidencia que falta.')}</span>`}
};
const DEFAULT_VENDOR_COLUMNS=['vendor','decision','opportunity','competitors','channel','ecosystem','analyst','synergy','overlap','confidence','gate'];
function initVendorColumns(){try{const saved=JSON.parse(localStorage.getItem('westcon-v3-columns')||'[]');state.vendorColumns=saved.filter(x=>VENDOR_COLUMNS[x]);}catch{state.vendorColumns=[]}if(!state.vendorColumns.length)state.vendorColumns=[...DEFAULT_VENDOR_COLUMNS];if(!state.vendorColumns.includes('vendor'))state.vendorColumns.unshift('vendor');state.vendorSort=localStorage.getItem('westcon-v3-sort')||'decisionScore';state.vendorSortDirection=localStorage.getItem('westcon-v3-sort-dir')||'desc'}
function renderTableStudio(){const picker=$('#columnPicker'),sort=$('#vendorSort');if(!picker||!sort)return;picker.innerHTML=`<small>Elige campos. Puedes arrastrar las cabeceras de la tabla para cambiar el orden.</small>`+Object.entries(VENDOR_COLUMNS).map(([id,c])=>`<label><input type="checkbox" data-column="${id}" ${state.vendorColumns.includes(id)?'checked':''} ${id==='vendor'?'disabled':''}> ${esc(c.label)}</label>`).join('');$$('#columnPicker input').forEach(x=>x.onchange=()=>{x.checked?state.vendorColumns.push(x.dataset.column):state.vendorColumns=state.vendorColumns.filter(c=>c!==x.dataset.column);localStorage.setItem('westcon-v3-columns',JSON.stringify(state.vendorColumns));renderVendorTable()});sort.innerHTML=`<option value="decisionScore">Prioridad global</option>`+Object.entries(VENDOR_COLUMNS).map(([id,c])=>`<option value="${id}" ${state.vendorSort===id?'selected':''}>${esc(c.label)}</option>`).join('');sort.value=state.vendorSort;$('#vendorSortDirection').textContent=state.vendorSortDirection==='desc'?'↓ Descendente':'↑ Ascendente'}
function vendorSortValue(v,key){return key==='decisionScore'?v.analysis.decisionScore:(VENDOR_COLUMNS[key]?.sort(v)??0)}
function bindColumnDrag(){let dragged='';$$('#vendorTableHead th').forEach(th=>{th.ondragstart=()=>{dragged=th.dataset.column;th.classList.add('dragging')};th.ondragend=()=>th.classList.remove('dragging');th.ondragover=e=>e.preventDefault();th.ondrop=e=>{e.preventDefault();const target=th.dataset.column;if(!dragged||dragged===target)return;const cols=state.vendorColumns.filter(x=>x!==dragged),at=cols.indexOf(target);cols.splice(at,0,dragged);state.vendorColumns=cols;localStorage.setItem('westcon-v3-columns',JSON.stringify(cols));renderTableStudio();renderVendorTable()}})}
function selectedVendorColumns(limit=99){return state.vendorColumns.filter(x=>VENDOR_COLUMNS[x]).slice(0,limit)}
function scoreCell(v,key,cls=''){const x=v.analysis[key];return `<div class="score-cell ${cls}">${derivedIntel(`<b>${x}</b>` ,VENDOR_COLUMNS[key]?.label||key,v,`Inferencia ponderada del motor; variable ${key}`)}<span class="scorebar"><i style="--w:${x}%"></i></span></div>`}
...
function renderOverviewVendors(){
  const rows=[...state.vendors].sort(verifiedDecisionSort).slice(0,12);
  $('#overviewVendorRows').innerHTML=rows.map(v=>`<tr data-vendor="${esc(v.name)}"><td><span class="vendor-name">${esc(v.name)}</span><span class="tiny">${esc(v.domain)}</span></td><td><span class="priority ${v.analysis.recommendation}">${esc(v.analysis.recommendation)}</span><span class="tiny">confianza ${v.analysis.recommendationConfidence}/100</span></td><td>${tags(v.marketCompetitors,'',3)}</td><td>${channelSummary(v)}</td><td>${ecosystemSummary(v)}</td><td>${analystSummary(v)}</td><td>${synergyOverlap(v)}</td><td>${esc(v.analysis.v34Recommendation?.action||'Investigar la información prioritaria que falta.')}</td></tr>`).join('');
  $$('#overviewVendorRows tr').forEach(tr=>tr.onclick=()=>{switchView('fabricantes');selectVendor(tr.dataset.vendor)})
}
function renderVendorTable(){
  const direction=state.vendorSortDirection==='desc'?-1:1,rows=filteredVendors().sort((a,b)=>{const av=vendorSortValue(a,state.vendorSort),bv=vendorSortValue(b,state.vendorSort);return (typeof av==='string'?String(av).localeCompare(String(bv)):Number(av)-Number(bv))*direction||a.name.localeCompare(b.name)}),cols=selectedVendorColumns();$('#resultCount').textContent=`${rows.length} fabricantes`;
  $('#vendorTableHead').innerHTML=cols.map(id=>`<th draggable="true" data-column="${id}" title="Arrastrar para mover">${esc(VENDOR_COLUMNS[id].label)}</th>`).join('');
  $('#vendorRows').innerHTML=rows.map(v=>`<tr data-vendor="${esc(v.name)}" class="${state.selected===v.name?'selected':''}">${cols.map(id=>`<td data-column="${id}">${VENDOR_COLUMNS[id].html(v)}</td>`).join('')}</tr>`).join('');
  bindColumnDrag();
...
function gateQuestions(v){return realityFor(v)?.action?.internalQuestions||['pipeline cuantificado por país','partner, integrador y ruta de compra','capacidad técnica y certificaciones activas','margen, attach de servicios y modelo económico','responsable, presupuesto y fecha del siguiente gate']}
function gateKey(v){return `westcon-v2-gate-${norm(v.name).replaceAll(' ','-')}`}
function gateData(v){try{return JSON.parse(localStorage.getItem(gateKey(v))||'{}')}catch{return {}}}
function gateReadiness(v){const data=gateData(v),questions=gateQuestions(v),answered=questions.filter((_,i)=>String(data[i]||'').trim()).length;return {answered,total:questions.length,missing:questions.filter((_,i)=>!String(data[i]||'').trim())}}
function internalGateHtml(v){const data=gateData(v),ready=gateReadiness(v);return `<div class="internal-gate"><div class="gate-head"><div><span>GATE INTERNO · NO ALTERA LA EVIDENCIA PÚBLICA</span><b>Datos necesarios para autorizar inversión</b></div><strong class="gate-score">${ready.answered}/${ready.total}</strong></div><p class="gate-missing">${ready.missing.length?`Pendiente: ${esc(ready.missing.join(' · '))}`:'Gate completo para revisión humana. No implica aprobación automática.'}</p><div class="gate-grid">${gateQuestions(v).map((q,i)=>`<label><span>${i+1}. ${esc(q)}</span><input data-gate-index="${i}" value="${esc(data[i]||'')}" placeholder="Dato interno, cifra o responsable"></label>`).join('')}</div><small>Se guarda únicamente en este navegador. El motor público no inventa ni estima estos datos.</small></div>`}
function bindInternalGate(v){const root=$('#vendorDetail');$$('[data-gate-index]',root).forEach(input=>input.addEventListener('input',()=>{const data=gateData(v);data[input.dataset.gateIndex]=input.value;localStorage.setItem(gateKey(v),JSON.stringify(data));const ready=gateReadiness(v);$('.gate-score',root).textContent=`${ready.answered}/${ready.total}`;$('.gate-missing',root).textContent=ready.missing.length?`Pendiente: ${ready.missing.join(' · ')}`:'Gate completo para revisión humana. No implica aprobación automática.'}))}
function realityContractHtml(v){
  const rec=v.analysis.v34Recommendation;
  if(!rec)return `<section class="reality-contract blocked"><div class="contract-head"><span>INVESTIGAR · SEÑAL PENDIENTE</span><b>La ausencia de prueba no demuestra ausencia de oportunidad</b></div><p>Obtener evidencia primaria sobre relación, alcance Iberia, capacidad del partner y economics internos antes de formular una acción comercial.</p></section>`;
  const sources=(rec.evidence||[]).map(e=>`<a href="${e.url}" target="_blank" rel="noopener">${esc(e.source)} · ${fmtDate(e.date)} · ${esc(e.fact)} ↗</a>`).join('');
  return `<section class="reality-contract v340-contract"><div class="contract-head"><span>${esc(rec.action_type)} · IMPACTO ${esc(rec.impact_potential?.band||'—').toUpperCase()}</span><b>${esc(rec.proposed_owner)} · ${esc(rec.horizon)}</b></div><p class="contract-thesis">${esc(rec.action)}</p><div class="contract-timeline"><div><b>Por qué</b><p>${esc(rec.why)}</p></div><div><b>Por qué ahora</b><p>${esc(rec.why_now)}</p></div><div><b>Qué falta</b><p>${esc((rec.missing_information||[]).slice(0,4).join(' · '))}</p></div></div><div class="contract-kpis"><b>Confianza</b><span>Hechos ${Math.round(100*(rec.fact_confidence?.score||0))}/100</span><span>Interpretación ${Math.round(100*(rec.interpretation_confidence?.score||0))}/100</span><span>Riesgo de acción ${Math.round(100*(rec.action_risk?.score||0))}/100</span></div><div class="contract-gate"><b>Qué la cambiaría</b><span>${esc(rec.evidence_that_would_change_recommendation)}</span></div><details><summary>${(rec.evidence||[]).length} fuentes que sostienen esta recomendación</summary>${sources}</details></section>`
}
function selectVendor(name){
  ...
  $('#vendorDetail').innerHTML=`<div class="detail-top"><div class="domain">${esc(v.domain)} · ${v.countries.join(' / ')}</div><h2>${esc(v.name)}</h2><div class="decision-line"><span class="priority ${a.recommendation}">${a.recommendation}</span><b>${a.recommendationConfidence}<small>/100 confianza decisión</small></b></div><div style="margin-top:8px">${tags(v.capabilities)}</div></div>
  ${realityContractHtml(v)}
  ...
  <div class="action-box"><b>${esc(a.recommendation)} · ACCIÓN PROPORCIONAL</b><p>${esc(a.v34Recommendation?.action||'Investigar y documentar la evidencia que falta antes de proponer una acción de mayor riesgo.')}</p></div>
  ${internalGateHtml(v)}
  ${a.actionAuthorized?`<div class="detail-block"><h4>ACCIONES INTERNAS CANDIDATAS</h4><div class="action-stack">${execActs}</div></div><div class="detail-block"><div class="role-picker"><h4>RECOMENDACIONES POR PERFIL</h4><select id="roleSelect">${roleOpts}</select></div><div id="roleRecommendations">${roleRecommendationHtml(v,state.selectedRole)}</div></div>`:'<div class="method-note"><b>El tipo de acción limita el riesgo.</b> Validar, investigar o vigilar sí son acciones útiles; no autorizan inversión ni sustituyen la validación humana e interna.</div>'}
...
function renderEngine(){
  ...
  const w=state.engine.opportunityWeights,u=state.universe||{};$('#engineExplanation').innerHTML=`<p><b>Motor v12 acotado, adaptativo y reanudable.</b> Investiga ${u.analysts?.length||0} consultoras, ${u.distributors?.length||0} mayoristas y ${u.integrators?.length||0} integradores semilla, además de actores descubiertos dinámicamente. Cada ejecución usa un presupuesto estricto de tiempo: procesa lotes pequeños, publica lo válido, conserva la cola pendiente y continúa en el ciclo siguiente.</p><p>Aprende autoridad, disponibilidad, latencia, rendimiento, precisión ES/PT, relación nueva, cifras, corroboración y frescura por consulta, entidad, dimensión, país y tipo de fuente.</p><p><b>Paradigma v3.4:</b> separa confianza del hecho, confianza de la interpretación y riesgo de la acción. Una señal material se transforma en Actuar, Preparar/Validar, Investigar o Vigilar; solo se descarta cuando no merece atención ejecutiva y la disposición queda auditada.</p><div class="weight-list">${Object.entries(w).map(([k,v])=>`<div><span>${esc(labels[k]||k)}</span><b>${Math.round(v*100)}%</b><i style="--w:${v*100}%"></i></div>`).join('')}</div><p class="tiny">Evidencia pública y datos internos permanecen separados. Los datos ausentes se muestran como información pendiente; nunca se estiman en silencio.</p>`
}
...
function scheduleSummary(){const s=state.schedule||{},p=s.profiles||{},days=['lunes','martes','miércoles','jueves','viernes','sábado','domingo'],label=(x,text)=>x?.enabled===false?`${text} desactivada`:`${text} ${x?.time||'—'}`;return `${s.timezone||'zona no configurada'} · ${label(p.daily,'diaria')} · ${p.deep?.enabled===false?'semanal desactivada':`${days[Number(p.deep?.weekday||0)]} ${p.deep?.time||'—'}`} · ${p.exhaustive?.enabled===false?'mensual desactivada':`día ${p.exhaustive?.day||'—'} ${p.exhaustive?.time||'—'}`}`}
...
Run ID: ${r.runId||st.runId||'baseline'}
Resultado: ${r.runOutcome||st.outcome||'baseline'}
Consultas planificadas: ${r.queryCount||st.queryCount||0}
Evidencias públicas unificadas: ${allEvidence().length} (${state.reality?.facts?.length||0} realidades verificadas)
...
Modo gratuito sin claves: ${r.freeOnly===false?'NO':'sí'}
Tareas masivas: ${r.derived?.resilience?.plannedTasks||st.resilience?.plannedTasks||0}
Reanudadas desde checkpoint: ${r.derived?.resilience?.resumedTasks||st.resilience?.resumedTasks||0}
Estrategias aprendidas: ${r.derived?.resilience?.learningStrategies||st.resilience?.learningStrategies||0}
Tareas pendientes: ${r.derived?.resilience?.pendingTasks||st.resilience?.pendingTasks||0}
Fallos trazados: ${r.derived?.runErrorCount||st.errorCount||0}
Universo: ${state.universe?.analysts?.length||0} consultoras · ${state.universe?.distributors?.length||0} mayoristas · ${state.universe?.integrators?.length||0} integradores

Autoactualización: ${scheduleSummary()}. Configurable con scripts/configure_updates.py.
Ejecución acotada por tiempo, incremental, reanudable y tolerante a fallos por etapa.
Fuentes: webs/sitemaps oficiales, Common Crawl (descubrimiento + revalidación), Google News RSS, GDELT, Arquivo.pt, TED, PLACSP, recursos públicos de dados.gov.pt y páginas públicas de analistas.
...
function renderOperations(){
  const m=(state.runManifest?.runId?state.runManifest:state.research?.derived?.runStages?{runId:state.research.runId,outcome:state.research.runOutcome,stages:state.research.derived.runStages,errorCount:state.research.derived.runErrorCount}:{}),s=state.supervisor||state.status?.supervisor||{},res=state.status?.resilience||state.research?.derived?.resilience||{},outcome=s.outcome&&!['not-run-yet'].includes(s.outcome)?s.outcome:(m.outcome||state.status?.outcome||'baseline'),cls=/failed/i.test(outcome)?'bad':/partial|degraded|preserved/i.test(outcome)?'warn':'';
  const cards=[['Resultado',outcome,cls,'Completa, parcial recuperable o último dataset conservado'],['Run ID',m.runId||s.runId||state.research?.runId||'baseline','',`Perfil ${m.profile||s.profileRequested||state.status?.profile||'baseline'}`],['Duración',`${Math.round(Number(s.elapsedSeconds||m.elapsedSeconds||0))} s`,'',`Presupuesto ${m.maxRuntimeSeconds||state.status?.budgets?.max_runtime_seconds||'—'} s`],['Pendientes',String(res.pendingTasks||0),res.pendingTasks?'warn':'','Se conservan en checkpoint para la siguiente ejecución'],['Fallos',String(m.errorCount||state.status?.errorCount||0),m.errorCount?'warn':'',s.lastGoodRestored?'Se restauró el último dataset válido':'Sin pérdida del dataset publicado']];
  $('#runHealth').innerHTML=cards.map(x=>`<div class="${x[2]}"><span>${esc(x[0])}</span><b>${esc(x[1])}</b><small>${esc(x[3])}</small></div>`).join('');
  const stages=m.stages||state.research?.derived?.runStages||[];$('#runStages').innerHTML=stages.length?stages.map(x=>`<div class="stage-row ${esc(x.status)}"><div><b>${esc(x.name)}</b><span>${Number(x.durationSeconds||0).toFixed(1)} s${x.signals!==undefined?` · ${x.signals} señales`:''}</span></div><strong>${esc(String(x.status||'—').toUpperCase())}</strong></div>`).join(''):'<span class="tiny">La primera ejecución de v3.4 rellenará la cronología de etapas.</span>';
  const sources=Object.entries(state.sourceHealth?.sources||{}).map(([domain,x])=>({domain,...x})).sort((a,b)=>Number(b.failures||0)-Number(a.failures||0)||Number(b.latencyMsEma||0)-Number(a.latencyMsEma||0)).slice(0,12);$('#sourceHealth').innerHTML=sources.length?sources.map(x=>`<div class="health-source-row"><div><b>${esc(x.domain)}</b><span>${x.attempts||0} intentos · ${x.successes||0} OK · ${x.usefulHits||0} hallazgos</span></div><strong>${Math.round(Number(x.successRate||0)*100)}% · ${x.latencyMsEma||0} ms</strong></div>`).join(''):'<span class="tiny">La telemetría por dominio aparecerá tras la primera actualización.</span>';
  const errors=(state.researchErrors?.errors||[]).slice().reverse().slice(0,100);$('#failureRows').innerHTML=errors.length?errors.map(x=>`<tr><td>${fmtDate(x.at)}</td><td><span class="tiny">${esc(x.runId||'—')}</span></td><td><b>${esc(x.stage||'—')}</b></td><td>${esc(x.source||'—')}</td><td>${esc(x.errorType||'Error')}<span class="tiny">${esc(x.message||'')}</span></td><td>${x.recoverable?'Sí':'No'}</td></tr>`).join(''):'<tr><td colspan="6">Sin fallos registrados. La tabla se alimentará automáticamente si una fuente o etapa falla.</td></tr>'
}
function moduleOn(id){return state.reportModules.has(id)}
function roleSummaryRows(v){const roles=['country_manager','director_vsm_sa','director_psm','vsm','psm','sa','marketing','services','operations','finance','logistics'];return roles.map(id=>{const r=(state.decision.roles||[]).find(x=>x.id===id),a=v.derived.roleRecommendations?.[id]?.[0];return a?{role:r?.name||id,action:a.name,score:a.score,value:actionPlainLanguage(v,a,id)}:null}).filter(Boolean)}
function executiveReportTitle(){return ($('#reportTitle')?.value||'Radar Estratégico Tecnológico · Westcon Iberia').trim()}
function reportHtml(){
  if(window.WestconV340?.reportHtml)return window.WestconV340.reportHtml(state,executiveReportTitle());
  const top=[...state.vendors].sort(verifiedDecisionSort).slice(0,15),authorized=top.filter(v=>v.analysis.actionAuthorized),parts=[],title=executiveReportTitle(),cols=selectedVendorColumns(12);
  parts.push(`<div class="report-export"><section class="report-cover"><div class="eyebrow">WESTCON IBERIA · FY27–FY30</div><h1>${esc(title)}</h1><p>Inteligencia de fabricantes, mayoristas, integradores, mercado y arquitecturas con trazabilidad de fuente, fecha, geografía y confianza.</p><div class="cover-meta">Motor v11 · ${esc(state.research?.runOutcome||state.status?.outcome||'baseline')} · ${fmtDate(state.research?.generatedAt||state.status?.generatedAt)} · Run ${esc(state.research?.runId||state.status?.runId||'baseline')}</div></section><div class="report-body">`);
  if(moduleOn('executive'))parts.push(`<section class="report-module"><div class="report-section-head"><h2>Resumen ejecutivo</h2><small>${authorized.length} acciones priorizadas de ${state.vendors.length}</small></div><p>El tipo de acción se ajusta a la confianza de los hechos, la interpretación y el riesgo; toda recomendación conserva su evidencia vinculada.</p>${authorized.length?authorized.slice(0,6).map(v=>`<h3>${esc(v.name)} · ${esc(v.analysis.recommendation)}</h3><p><b>Responsable: ${esc(v.derived.plan.owner)}</b><br>${esc(v.derived.plan.thesis)}<br><b>30:</b> ${esc(v.derived.plan.p30)}<br><b>60:</b> ${esc(v.derived.plan.p60)}<br><b>90:</b> ${esc(v.derived.plan.p90)}<br><b>Criterio:</b> ${esc(v.derived.plan.goNoGo)}</p>`).join(''):'<div class="method-note"><b>No hay acciones priorizadas en esta vista.</b> Consulte las acciones de validar, investigar y vigilar del brief v3.4.</div>'}</section>`);
  if(moduleOn('portfolio'))parts.push(`<section class="report-module"><div class="report-section-head"><h2>Comparativa de fabricantes</h2><small>Campos elegidos en la aplicación</small></div><table><thead><tr>${cols.map(id=>`<th>${esc(VENDOR_COLUMNS[id].label)}</th>`).join('')}</tr></thead><tbody>${top.map(v=>`<tr>${cols.map(id=>`<td>${esc(VENDOR_COLUMNS[id].text(v))}</td>`).join('')}</tr>`).join('')}</tbody></table></section>`);
  if(moduleOn('roles'))parts.push(`<section class="report-module"><h2>Recomendaciones por perfil</h2>${authorized.length?authorized.slice(0,8).map(v=>`<h3>${esc(v.name)}</h3><div class="report-role-grid">${roleSummaryRows(v).slice(0,8).map(x=>`<div><b>${esc(x.role)} · ${x.score}</b><p>${esc(x.action)} — ${esc(x.value)}</p></div>`).join('')}</div>`).join(''):'<p>Las acciones se presentan en el brief v3.4 con responsable, riesgo, confianza y evidencia.</p>'}</section>`);
  ...
  if(moduleOn('methodology'))parts.push(`<section class="report-module"><h2>Metodología</h2><p>Decision Intelligence v3.4: investigación adaptativa, checkpoints reanudables, aprendizaje de fuentes, trazabilidad y publicación atómica. Cada recomendación exige evidencia enlazada y una fuerza de acción compatible con su confianza y riesgo.</p></section>`);
  parts.push('</div></div>');return parts.join('')
}
async function exportPdf(){if(!window.html2pdf){toast('Librería PDF no disponible');return}const report=$('#report');report.innerHTML=reportHtml();report.style.display='block';await html2pdf().set({margin:0,filename:'Westcon_Iberia_Decision_Intelligence_v3.4.0.pdf',image:{type:'jpeg',quality:.98},html2canvas:{scale:1.8,useCORS:true,letterRendering:true},jsPDF:{unit:'mm',format:'a4',orientation:'landscape'},pagebreak:{mode:['css','legacy'],before:'.report-cover'}}).from(report.firstElementChild).save();report.style.display='none';toast('PDF ejecutivo generado')}
async function exportPptx(){
  if(window.WestconV340?.exportPptx)return window.WestconV340.exportPptx(state,executiveReportTitle(),toast);
  if(!window.PptxGenJS){toast('Librería PowerPoint no disponible');return}const pptx=new PptxGenJS();pptx.layout='LAYOUT_WIDE';pptx.author='Westcon Iberia Strategy Studio';pptx.title=executiveReportTitle();pptx.subject='Decision Intelligence FY27–FY30';pptx.company='Westcon-Comstor';pptx.lang='es-ES';pptx.theme={headFontFace:'Aptos Display',bodyFontFace:'Aptos',lang:'es-ES'};pptx.defineSlideMaster({title:'MASTER',background:{color:'FFFFFF'},objects:[{rect:{x:0,y:0,w:13.333,h:.18,fill:{color:'F09E0D'},line:{color:'F09E0D'}}},{text:{text:'WESTCON IBERIA · DECISION INTELLIGENCE',options:{x:.55,y:.25,w:6,h:.25,fontFace:'Aptos',fontSize:9,bold:true,color:'3195BB'}}},{text:{text:`Motor v11 · ${state.research?.runOutcome||'baseline'}`,options:{x:9.2,y:.25,w:3.5,h:.25,fontFace:'Aptos',fontSize:8,color:'687B8D',align:'right'}}}],slideNumber:{x:12.75,y:7.12,color:'687B8D',fontSize:8}});const addTitle=(s,t,sub='')=>{s.addText(t,{x:.55,y:.65,w:12.2,h:.55,fontFace:'Aptos Display',fontSize:27,bold:true,color:'082335',margin:0});if(sub)s.addText(sub,{x:.55,y:1.24,w:11.9,h:.4,fontFace:'Aptos',fontSize:11,color:'687B8D',margin:0})};const top=[...state.vendors].sort(verifiedDecisionSort).slice(0,14),authorized=top.filter(v=>v.analysis.actionAuthorized);let s;
  if(moduleOn('executive')){s=pptx.addSlide('MASTER');addTitle(s,executiveReportTitle(),'Acciones proporcionales a evidencia, interpretación y riesgo.');const k=[[state.vendors.length,'fabricantes'],[allEvidence().length,'evidencias'],[authorized.length,'acciones priorizadas'],[(state.universe?.integrators||[]).length,'integradores seguidos']];k.forEach((x,i)=>{s.addShape(pptx.ShapeType.roundRect,{x:.55+i*3.05,y:2,w:2.8,h:1.15,rectRadius:.05,fill:{color:i%2?'F7F9FA':'F2F7F8'},line:{color:'DBE4E9'}});s.addText(String(x[0]),{x:.75+i*3.05,y:2.2,w:2.3,h:.42,fontFace:'Aptos Display',fontSize:24,bold:true,color:'082335'});s.addText(x[1],{x:.75+i*3.05,y:2.68,w:2.3,h:.24,fontFace:'Aptos',fontSize:9,color:'687B8D'})});const message=authorized.length?authorized.slice(0,4).map(v=>`${v.name} · ${v.analysis.recommendation} · ${v.derived.plan.owner}\n90 días: ${v.derived.plan.p90}`).join('\n\n'):'Consulte el brief v3.4 para acciones de validar, investigar y vigilar con evidencia enlazada.';s.addText(message,{x:.65,y:3.45,w:11.7,h:3.0,fontFace:'Aptos',fontSize:11,bold:true,color:'113A50',breakLine:true,margin:0})}
  if(moduleOn('portfolio')){s=pptx.addSlide('MASTER');addTitle(s,'Comparativa de fabricantes','Campos de negocio elegidos en la tabla; las columnas internas y sin cobertura se ocultan.');const cols=selectedVendorColumns(8),rows=[[...cols.map(id=>VENDOR_COLUMNS[id].label)],...top.map(v=>cols.map(id=>VENDOR_COLUMNS[id].text(v)))];s.addTable(rows,{x:.25,y:1.55,w:12.8,h:5.55,border:{type:'solid',color:'D9E2E7',pt:.6},fontFace:'Aptos',fontSize:7,color:'233746',fill:'FFFFFF',margin:.035})}
  if(moduleOn('roles')){for(const v of authorized.slice(0,6)){s=pptx.addSlide('MASTER');addTitle(s,`${v.name} · acciones por perfil`,`${v.derived.archetype.name} · ${v.analysis.recommendation} · evidencia trazable`);const rr=roleSummaryRows(v).slice(0,10);const rows=[['Perfil','Score','Acción','Por qué'],...rr.map(x=>[x.role,String(x.score),x.action,x.value])];s.addTable(rows,{x:.35,y:1.55,w:12.5,h:5.4,border:{type:'solid',color:'D9E2E7',pt:.6},fontFace:'Aptos',fontSize:7.4,color:'233746',fill:'FFFFFF',margin:.035,colW:[2.0,.55,3.0,6.95]})}}
  ...
  if(moduleOn('synergies')){s=pptx.addSlide('MASTER');addTitle(s,'Sinergias, solapes y fronteras técnicas','Arquitecturas candidatas para validar; una combinación de fabricantes no implica recomendación automática.');(state.base.solutionPlays||[]).slice(0,6).forEach((p,i)=>{const col=i%3,row=Math.floor(i/3),x=.55+col*4.12,y=1.75+row*2.42;s.addShape(pptx.ShapeType.rect,{x,y,w:3.78,h:2.05,fill:{color:'082335'},line:{color:'082335'}});s.addText(p.name,{x:x+.18,y:y+.25,w:3.35,h:.42,fontFace:'Aptos Display',fontSize:16,bold:true,color:'FFFFFF',margin:0});s.addText(p.vendors.join(' + '),{x:x+.18,y:y+.82,w:3.35,h:.4,fontFace:'Aptos',fontSize:7.5,color:'12C7C0',margin:0});s.addText(p.value,{x:x+.18,y:y+1.25,w:3.35,h:.55,fontFace:'Aptos',fontSize:8,color:'CDD6E0',margin:0})})}
  ...
  if(moduleOn('methodology')){s=pptx.addSlide('MASTER');addTitle(s,'Metodología','Simplicidad visible; evidencia, actualización y fallos trazables.');s.addText('Dimensiones estratégicas\n+ aprendizaje por rendimiento de fuente\n+ investigación adaptativa\n+ checkpoint reanudable\n+ corroboración y geografía\n+ acción proporcional al riesgo\n+ diagnóstico por etapa',{x:.65,y:1.8,w:4.0,h:3.7,fontFace:'Aptos Display',fontSize:18,bold:true,color:'082335',margin:0,breakLine:true});s.addText('El motor v3.4 separa confianza factual, confianza interpretativa y riesgo. Publica resultados parciales válidos, mantiene una cola pendiente, conserva el último dataset correcto si una ejecución falla y registra etapa, fuente, latencia y recuperación.',{x:5.0,y:1.8,w:7.3,h:3.3,fontFace:'Aptos',fontSize:14,color:'445D6C',margin:0})}
  await pptx.writeFile({fileName:'Westcon_Iberia_Decision_Intelligence_v3.4.0.pptx'});toast('PowerPoint ejecutivo generado')
}
