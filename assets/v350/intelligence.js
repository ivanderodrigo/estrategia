(() => {
  'use strict';
  const state = {data:null, view:'fabricantes'};
  ...
  async function load(){
    const res = await fetch('data/v35/intelligence.json', {cache:'no-store'});
    if(!res.ok) throw new Error(`No se pudo cargar data/v35/intelligence.json (${res.status})`);
  ...
  function bind(){
    ...
    $('#manufacturerRole')?.addEventListener('change', renderManufacturers);
  ...
  function renderValue(field){
    ...
    if(Array.isArray(value)){
      ...
      return `<div class="stack">${value.slice(0,8).map(v=>`<div class="line">${esc(v)}</div>`).join('')}${value.length>8?`<small>+${value.length-8} más</small>`:''}</div>`;
    }
  ...
  function renderManufacturers(){
    const q=norm($('#manufacturerSearch')?.value), role=$('#manufacturerRole')?.value||'all';
    const all=state.data.manufacturers||[];
    const rows=all.filter(r=>(!q||rowBlob(r).includes(q))&&(role==='all'||r.fields?.portfolio_role?.value===role));
  ...
  function renderIntegrators(){
    const q=norm($('#integratorSearch')?.value), scope=$('#integratorScope')?.value||'all';
    const all=state.data.integrators||[];
    const rows=all.filter(r=>(!q||rowBlob(r).includes(q))&&scopeMatch(r,scope));
    $('#integratorTable').innerHTML=tableHtml(rows,state.data.schemas.integrators,'No hay integradores con esos filtros.');
    setCount('#integratorCount',rows.length,all.length,'integradores');
  }
  ...
  async function exportPdf(){
    ...
    try{
      if(window.html2pdf){ await window.html2pdf().set({margin:0,filename:'Westcon_Iberia_Business_Intelligence_v3.5.0.pdf',image:{type:'jpeg',quality:.97},html2canvas:{scale:1.5,useCORS:true},jsPDF:{unit:'mm',format:'a4',orientation:'landscape'},pagebreak:{mode:['css','legacy']}}).from(sheet.firstElementChild).save(); toast('PDF generado'); }
      else { window.print(); }
  ...
  async function exportPptx(){
    ...
    await pptx.writeFile({fileName:'Westcon_Iberia_Business_Intelligence_v3.5.0.pptx'}); closeModal('exportModal'); toast('PowerPoint generado');
  }

  function renderAll(){
    renderManufacturers(); renderIntegrators(); renderDistributors(); renderTrends(); renderArchitectures(); renderSourceCatalog();
    const meta=state.data.meta||{}; const status=$('#footerStatus'); if(status) status.textContent=`v${meta.version||'3.5.0'} · ${meta.source_count||0} fuentes/familias · ${meta.scope||'Iberia'}`;
  }
