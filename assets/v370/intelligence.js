(() => {
  ...
  async function load(){
    const res = await fetch('data/v37/intelligence.json', {cache:'no-store'});
    if(!res.ok) throw new Error(`No se pudo cargar data/v37/intelligence.json (${res.status})`);
  ...
  function tagHtml(field,value,index,extra=''){
    ...
    return traceable(field,`<span class="tag confidence-tag ${esc(band)} ${extra}">${esc(typeof value==='object'?JSON.stringify(value):value)}<small>${confidencePct(score)}%</small></span>`,item);
  }
  function renderValue(field, limit=5){
    const value=field?.value;
    if(Array.isArray(value)){
      if(value.length&&typeof value[0]==='object'&&value[0].layer){
        return `<div class="layer-list">${value.map(x=>{const layerField={...field,items:x.vendor_items||[]};return `<div class="layer"><b>${esc(x.layer)}</b><div>${(x.vendors||[]).map((v,i)=>tagHtml(layerField,v,i,'emphasis')).join('')}</div>${x.note?`<small>${esc(x.note)}</small>`:''}</div>`;}).join('')}</div>`;
      }
      const tags=value.map((v,i)=>`<span class="tag-entry ${i>=limit?'tag-overflow':''}">${tagHtml(field,v,i)}</span>`).join('');
      const more=value.length>limit?`<button type="button" class="more-tags" data-more-tags data-label="… +${value.length-limit}">… +${value.length-limit}</button>`:'';
      return `<div class="tag-list">${tags}${more}</div>`;
    }
    return traceable(field,esc(value));
  }
  function headerCell(col, view){
    const sort=state.sort[view], arrow=sort?.col===col.id?(sort.dir===1?' ↑':' ↓'):'';
    return `<th draggable="true" data-col="${esc(col.id)}" title="Arrastrar para mover · clic para ordenar"><span class="drag-grip">⋮⋮</span><span class="help-wrap"><span>${esc(col.label)}${arrow}</span>${col.clarify?`<button class="help-icon" type="button" aria-label="Aclaración de ${esc(col.label)}">?</button><span class="help-tip">${esc(col.help||'')}</span>`:''}</span></th>`;
  }
  function activeColumns(schema, rows, view){
    let cols=schema.filter(col=>rows.some(row=>hasValue(row.fields?.[col.id]?.value)));
  ...
  function tableHtml(rows, schema, emptyText, view){
    ...
    return `<table data-view="${esc(view)}"><thead><tr><th class="entity-head">Entidad</th>${cols.map(c=>headerCell(c,view)).join('')}</tr></thead><tbody>${ordered.map(row=>{const identity={value:row.name,evidence:row.evidence||[],confidence:.95,confidence_band:'high'};return `<tr><td class="name-cell">${traceable(identity,`<b>${esc(row.name)}</b><small>Identidad trazable</small>`)}</td>${cols.map(col=>{const f=row.fields?.[col.id];return `<td>${f&&hasValue(f.value)?renderValue(f,5):'<span class="research-gap">En investigación</span>'}</td>`}).join('')}</tr>`}).join('')}</tbody></table>`;
  }
  ...
  function cardField(col,f){if(!f||!hasValue(f.value))return '';const help=col.clarify?`<span class="help-wrap card-help"><span>${esc(col.label)}</span><button class="help-icon" type="button" aria-label="Aclaración de ${esc(col.label)}">?</button><span class="help-tip">${esc(col.help||'')}</span></span>`:esc(col.label);return `<div class="card-field"><label>${help}</label>${renderValue(f,7)}</div>`;}
  ...
  function renderTrendAnalytics(rows){
    ...
    life.innerHTML=`<div class="chart-title"><div><span>WESTCON TREND PULSE</span><h3>Estado y velocidad de las tendencias</h3><p>Síntesis propia basada en evidencia. No reproduce Gartner Hype Cycle, Magic Quadrant ni Forrester Wave.</p></div><div class="chart-legend"><i class="portfolio-dot"></i> tamaño = urgencia comprador</div></div><div class="lifecycle-axis"><span>Emergente</span><span>Aceleración</span><span>Escala</span><span>Consolidación</span></div><div class="pulse-plot">${usable.map(r=>{const a=r.analytics||{},x=Math.max(2,Math.min(98,+a.maturity||0)),y=Math.max(4,Math.min(94,100-(+a.momentum||0))),sz=16+Math.round((+a.buyer_urgency||50)/8);return `<button class="trend-bubble" style="left:${x}%;top:${y}%;width:${sz}px;height:${sz}px" title="${esc(r.name)} · madurez ${a.maturity}% · momentum ${a.momentum}% · urgencia ${a.buyer_urgency}% · intensidad competitiva ${a.competitive_intensity}%"><span>${esc(r.name)}</span></button>`}).join('')}</div>`;
    const arr=trendActorData(32),maxT=Math.max(1,...arr.map(a=>a.trends.size)),maxE=Math.max(1,...arr.map(a=>a.evidence));
    map.innerHTML=`<div class="chart-title"><div><span>MAPA DE ACTORES × TENDENCIAS</span><h3>Amplitud temática y evidencia pública</h3><p>Compara presencia documentada, no liderazgo, cuota ni valoración comercial. Azul oscuro = portfolio Westcon; contorno = otros actores.</p></div></div><div class="actor-y">Más evidencia pública ↑</div><div class="actor-plot">${arr.map(a=>{const x=5+90*(a.trends.size/maxT),y=92-84*(a.evidence/maxE),cls=a.portfolio?'westcon':'external';return `<button class="actor-bubble ${cls}" style="left:${x}%;top:${y}%" title="${esc(a.name)} · ${a.trends.size} tendencias · ${a.evidence} evidencias"><span>${esc(a.name)}</span></button>`}).join('')}</div><div class="actor-x">Menor amplitud temática ← · → Mayor amplitud temática</div>`;
  }
  ...
  function reportFooter(label){return `<footer class="r-footer"><span>${esc(label)}</span><span>v${esc(state.data.meta.version||'3.7.0')} · ${esc(state.data.meta.scope||'Iberia')} · inteligencia trazable</span></footer>`;}
  function reportCover(title,modules){
    ...
    return `<section class="report-page report-cover">
      <div class="r-cover-top">${reportBrand()}<span class="r-version">v${esc(state.data.meta.version||'3.7.0')}</span></div>
  ...
  function reportTrendAnalyticsPage(){
    ...
    const pulse=`<section class="report-page r-section-page r-analytics-page" style="--accent:#${exportTheme.blue}"><div class="r-page-head"><div>${reportBrand()}<div class="r-eyebrow">WESTCON TREND PULSE</div><h2>Estado y velocidad de las tendencias</h2><p>Síntesis propia basada en evidencia: madurez × momentum; tamaño = urgencia del comprador. No reproduce Gartner Hype Cycle, Magic Quadrant ni Forrester Wave.</p></div></div><div class="r-axis-labels"><span>Emergente</span><span>Aceleración</span><span>Escala</span><span>Consolidación</span></div><div class="r-pulse-plot">${rows.map(r=>{const a=r.analytics||{},x=Math.max(3,Math.min(97,+a.maturity||0)),y=Math.max(5,Math.min(93,100-(+a.momentum||0))),sz=4.5+((+a.buyer_urgency||50)/100)*4;return `<div class="r-pulse-dot" style="left:${x}%;top:${y}%;width:${sz}mm;height:${sz}mm"><span>${esc(r.name)}</span></div>`}).join('')}</div><div class="r-analytics-foot">Eje X: madurez · Eje Y: momentum · burbuja: urgencia comprador · síntesis Westcon basada en las fuentes trazables de cada tendencia.</div>${reportFooter('Tendencias · Trend Pulse')}</section>`;
    const actor=`<section class="report-page r-section-page r-analytics-page" style="--accent:#${exportTheme.blue}"><div class="r-page-head"><div>${reportBrand()}<div class="r-eyebrow">MAPA DE ACTORES × TENDENCIAS</div><h2>Amplitud temática y evidencia pública</h2><p>Presencia documentada, no liderazgo, cuota ni valoración comercial. Azul oscuro = portfolio Westcon; contorno rosa = otros actores.</p></div></div><div class="r-actor-y">Más evidencia pública ↑</div><div class="r-actor-plot">${actors.map(a=>{const x=5+90*(a.trends.size/maxT),y=92-84*(a.evidence/maxE);return `<div class="r-actor-dot ${a.portfolio?'westcon':'external'}" style="left:${x}%;top:${y}%"><span>${esc(a.name)}</span></div>`}).join('')}</div><div class="r-analytics-foot">Menor amplitud temática ← · → Mayor amplitud temática · cada punto se calcula desde tendencias y evidencias publicadas en el dataset.</div>${reportFooter('Tendencias · Mapa de actores')}</section>`;
    return pulse+actor;
  }
  ...
  function pptAddFooter(slide,pptx,label,dark=false){
    ...
    slide.addText(`v${state.data.meta.version||'3.7.0'} · ${state.data.meta.scope||'Iberia'} · trazabilidad en la aplicación`,{x:7.0,y:7.1,w:5.75,h:.14,fontFace:'Aptos',fontSize:6.6,color,align:'right',margin:0});
  }
  ...
  function pptAddTrendAnalytics(pptx){
    const rows=state.data.trends||[]; let slide=pptx.addSlide();slide.background={color:exportTheme.bg};pptAddSlideTitle(slide,pptx,'Westcon Trend Pulse','Síntesis propia · madurez × momentum · tamaño de burbuja orientativo por urgencia',exportTheme.blue);
    const x0=1.2,y0=1.95,w=10.8,h=4.55;slide.addShape(pptx.ShapeType.rect,{x:x0,y:y0,w,h,fill:{color:'FFFFFF'},line:{color:exportTheme.line,pt:1}});
    [25,50,75].forEach(v=>slide.addShape(pptx.ShapeType.line,{x:x0+w*v/100,y:y0,w:0,h,line:{color:'DDE7EB',pt:.7,dash:'dash'}}));
    slide.addText('Momentum ↑',{x:.35,y:2.0,w:.7,h:.2,fontSize:6.5,bold:true,color:exportTheme.muted,rotate:270,margin:0});slide.addText('Madurez →',{x:6.0,y:6.62,w:1.3,h:.2,fontSize:6.5,bold:true,color:exportTheme.muted,margin:0});
    rows.forEach(r=>{const a=r.analytics||{},x=x0+w*Math.max(0,Math.min(100,+a.maturity||0))/100,y=y0+h*(1-Math.max(0,Math.min(100,+a.momentum||0))/100),sz=.14+.18*((+a.buyer_urgency||50)/100);slide.addShape(pptx.ShapeType.ellipse,{x:x-sz/2,y:y-sz/2,w:sz,h:sz,fill:{color:exportTheme.blue,transparency:8},line:{color:'FFFFFF',pt:.8}});slide.addText(r.name,{x:x-.55,y:y+sz/2,w:1.1,h:.22,fontSize:4.8,bold:true,color:exportTheme.ink,align:'center',margin:0,fit:'shrink'});});pptAddFooter(slide,pptx,'Tendencias · Trend Pulse',false);

    const actors=trendActorData(28),maxT=Math.max(1,...actors.map(a=>a.trends.size)),maxE=Math.max(1,...actors.map(a=>a.evidence));
    slide=pptx.addSlide();slide.background={color:exportTheme.bg};pptAddSlideTitle(slide,pptx,'Mapa de actores × tendencias','Amplitud temática × evidencia pública · presencia documentada, no liderazgo ni cuota',exportTheme.blue);
    const ax=1.15,ay=1.9,aw=10.9,ah=4.7;slide.addShape(pptx.ShapeType.rect,{x:ax,y:ay,w:aw,h:ah,fill:{color:'FFFFFF'},line:{color:exportTheme.line,pt:1}});[25,50,75].forEach(v=>{slide.addShape(pptx.ShapeType.line,{x:ax+aw*v/100,y:ay,w:0,h:ah,line:{color:'E2EAED',pt:.6,dash:'dash'}});slide.addShape(pptx.ShapeType.line,{x:ax,y:ay+ah*v/100,w:aw,h:0,line:{color:'E8EEF1',pt:.55,dash:'dash'}});});
    actors.forEach(a=>{const x=ax+aw*(.05+.9*(a.trends.size/maxT)),y=ay+ah*(.92-.84*(a.evidence/maxE)),c=a.portfolio?exportTheme.navy:exportTheme.white,l=a.portfolio?exportTheme.navy:exportTheme.pink;slide.addShape(pptx.ShapeType.ellipse,{x:x-.08,y:y-.08,w:.16,h:.16,fill:{color:c},line:{color:l,pt:1.1}});slide.addText(a.name,{x:x-.5,y:y+.1,w:1,h:.18,fontFace:'Aptos',fontSize:4.5,bold:true,color:exportTheme.ink,align:'center',margin:0,fit:'shrink'});});
    slide.addText('Más evidencia pública ↑',{x:.25,y:2.0,w:.75,h:.25,fontSize:6,bold:true,color:exportTheme.muted,rotate:270,margin:0});slide.addText('Menor amplitud temática ←      → Mayor amplitud temática',{x:4.4,y:6.67,w:4.6,h:.18,fontSize:6,bold:true,color:exportTheme.muted,align:'center',margin:0});
    slide.addShape(pptx.ShapeType.ellipse,{x:10.15,y:1.3,w:.13,h:.13,fill:{color:exportTheme.navy},line:{color:exportTheme.navy}});slide.addText('Westcon',{x:10.34,y:1.29,w:.7,h:.13,fontSize:5.5,bold:true,color:exportTheme.muted,margin:0});slide.addShape(pptx.ShapeType.ellipse,{x:11.12,y:1.3,w:.13,h:.13,fill:{color:exportTheme.white},line:{color:exportTheme.pink,pt:1}});slide.addText('Otros',{x:11.31,y:1.29,w:.7,h:.13,fontSize:5.5,bold:true,color:exportTheme.muted,margin:0});pptAddFooter(slide,pptx,'Tendencias · Mapa de actores',false);
  }
  async function exportPptx(){
    ...
    await pptx.writeFile({fileName:'Westcon_Iberia_Business_Intelligence_v3.7.0.pptx'}); closeModal('exportModal'); toast('PowerPoint generado con diseño Westcon');
  }
  ...
  function renderAll(){
    ...
    const meta=state.data.meta||{}; const status=$('#footerStatus'); if(status) status.textContent=`v${meta.version||'3.7.0'} · ${meta.source_count||0} fuentes/familias · ${meta.scope||'Iberia'}`;
  }
