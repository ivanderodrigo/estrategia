(() => {
  ...
  function traceable(field, inner, item=null){
    const evidence=(item===null?(field?.evidence||[]):(item?.evidence||[])).filter(ev=>/^https?:\/\//i.test(String(ev?.url||''))).slice(0,8);
    // Hard invariant: a value with no directly linked public URL is never rendered as
    // an interactive/traceable claim. The public projector normally removes it first;
    // this is a second defensive layer for identity/legacy data.
    if(!evidence.length) return `<span class="untraceable-value" title="Sin evidencia pública vinculada">${inner}</span>`;
    ...
    const sources=evidence.map(ev=>sourceItem(ev,factScore,band)).join('');
  ...
  function reportFooter(label){return `<footer class="r-footer"><span>${esc(label)}</span><span>v${esc(state.data.meta.version||'4.0.1')} · ${esc(state.data.meta.scope||'Iberia')} · inteligencia trazable</span></footer>`;}
  ...
  function reportCover(title,modules){
    ...
    return `<section class="report-page report-cover">
      <div class="r-cover-top">${reportBrand()}<span class="r-version">v${esc(state.data.meta.version||'4.0.1')}</span></div>
  ...
  function pptAddFooter(slide,pptx,label,dark=false){
    ...
    slide.addText(`v${state.data.meta.version||'4.0.1'} · ${state.data.meta.scope||'Iberia'} · trazabilidad en la aplicación`,{x:7.0,y:7.1,w:5.75,h:.14,fontFace:'Aptos',fontSize:6.6,color,align:'right',margin:0});
  }
  ...
  async function exportPptx(){
    ...
    await pptx.writeFile({fileName:`Westcon_Iberia_Business_Intelligence_v${state.data.meta.version||'4.0.1'}.pptx`});closeModal('exportModal');toast(appendix?'PowerPoint ejecutivo + anexo generado':'PowerPoint ejecutivo generado');
  }

  function renderAll(){
    populateIntegratorVendorFilter(); renderActiveView(); renderSourceCatalog(); renderUpdateStatus(); renderConfidenceGuide();
    const meta=state.data.meta||{}; const status=$('#footerStatus'); if(status) status.textContent=`App v4.0.1 · dataset v${meta.version||'4.0.1'} · ${meta.source_count||0} fuentes/familias · ${meta.scope||'Iberia'}`;
  }
