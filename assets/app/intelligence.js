(() => {
  ...
  function renderSourceCatalog(){
    ...
    $('#sourceCatalog').innerHTML=rows.slice(0,220).map(s=>`<div class="source-row"><b>${esc(s.name)}</b><small>${esc(s.class||'fuente / procedencia')} · ${(s.scope||[]).map(esc).join(' / ')}</small><div class="dims">${(s.dimensions||[]).slice(0,7).map(d=>`<span class="tag">${esc(d)}</span>`).join('')}</div>${s.url&&!String(s.url).startsWith('dynamic://')?`<a href="${esc(s.url)}" target="_blank" rel="noopener">Abrir ↗</a>`:(s.class==='WESTCON_DOCUMENT'?'<small>Documento Westcon · sin URL pública necesaria</small>':'<small>Enrutado dinámicamente por entidad</small>')}</div>`).join('') || '<div class="empty-state">Sin coincidencias.</div>';
  }
