...
(() => {
  ...
  function filteredReportHtml(model){
    ...
    return `<header><div><span>WESTCON IBERIA · DECISION INTELLIGENCE</span><h1>${esc(model.title)}</h1></div><b>v${esc(state.data.meta.version||'4.2.2')}</b></header><section class="filtered-report-meta"><div><b>Generado</b><span>${esc(localDateTime(model.generatedAt))}</span></div><div><b>Entidades</b><span>${model.entityCount}</span></div><div><b>Pendientes</b><span>${model.pending}</span></div></section><section><h2>Criterios de filtrado</h2>${reportCriteriaHtml(model)}</section>${summary?`<section><h2>Resumen ejecutivo derivado</h2><p>Resumen descriptivo calculado exclusivamente a partir de las ${model.entityCount} filas filtradas.</p><div class="filtered-summary">${derivedSummary(model)}</div></section>`:''}${model.pending?`<section class="filtered-warning"><h2>Advertencia de cobertura</h2><p>${model.pending} celdas incluidas están vacías o no tienen evidencia acreditativa vinculada. Se mantienen como «Por investigar» o «Pendiente de verificación».</p></section>`:''}${table}${confidence}${sources}<footer>Westcon Iberia · informe filtrado · ${esc(model.criteria)}</footer>`;
  }
  ...
  async function generateFilteredPptx(){
    ...
    await pptx.writeFile({fileName:`Westcon_Iberia_${model.view}_${model.entityCount}_resultados_v4.2.2.pptx`});closeModal('filteredReportModal');toast('PowerPoint generado con el subconjunto filtrado');
  }
  ...
  function renderAll(){
    ...
    const meta=state.data.meta||{}; const status=$('#footerStatus'); if(status) status.textContent=`App v4.2.2 · dataset v${meta.version||'4.2.2'} · ${meta.source_count||0} fuentes acreditativas/familias · ${meta.scope||'Iberia'}`;
  }
