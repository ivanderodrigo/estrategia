...
CSS=['  <link rel="stylesheet" href="assets/v332/ecosystem-intelligence.css?v=3.3.2">','  <link rel="stylesheet" href="assets/v332/executive-output.css?v=3.3.2">']
JS='  <script src="assets/v332/ecosystem-intelligence.js?v=3.3.2" defer></script>'

def backup(path):
 if not path.exists():return
 stamp=datetime.now().strftime('%Y%m%d%H%M%S');d=ROOT/'.local-backups'/f'v332-{stamp}';d.mkdir(parents=True,exist_ok=True);rel=path.relative_to(ROOT);dst=d/rel;dst.parent.mkdir(parents=True,exist_ok=True);shutil.copy2(path,dst)
...
def main():
 ...
 if 'assets/v332/ecosystem-intelligence.js' not in text:text=text.replace('</body>',JS+'\n</body>',1)
 INDEX.write_text(text,encoding='utf-8');VERSION.write_text('3.3.2\n',encoding='utf-8')
 if GI.exists():
 ...
 print('v3.3.2 aplicada: evidencia acumulativa + tiering T1/T2/T3 + objetivos de cobertura + intensidad de relación + cola de verificación.')
