#!/usr/bin/env python3
"""Transactional v3.14.0 -> v3.15.0 updater. No git mutations."""
from __future__ import annotations
import argparse,hashlib,json,os,shutil,subprocess,sys,tempfile
from pathlib import Path
HERE=Path(__file__).resolve().parent
PACKAGE=HERE if (HERE/'config/v315/update_manifest.json').is_file() else HERE.parent
MANIFEST=PACKAGE/'config/v315/update_manifest.json'
def parse_args():
 p=argparse.ArgumentParser(description='Aplica Westcon Decision Intelligence v3.15.0 sobre una copia v3.14.0');p.add_argument('--repo',required=True,help='Ruta a la raíz del repositorio v3.14.0');return p.parse_args()
...
def main()->int:
 ...
 if version!='3.14.0':print(f'ERROR: base incompatible. Se esperaba VERSION 3.14.0 y se encontró {version or "sin VERSION"}.',file=sys.stderr);return 3
 if not MANIFEST.is_file():print('ERROR: falta config/v315/update_manifest.json en el paquete.',file=sys.stderr);return 4
 ...
 backup=Path(tempfile.mkdtemp(prefix='westcon_v315_rollback_'));existing=[];created=[]
 try:
  for rel in files:
   ...
   dst.parent.mkdir(parents=True,exist_ok=True);tmp=dst.with_name(dst.name+'.v315.tmp');shutil.copy2(src,tmp);os.replace(tmp,dst)
  run_check([sys.executable,'-m','unittest','tests/test_v315.py','-v'],repo)
  run_check([sys.executable,'scripts/v315/validate_v315.py'],repo)
  if shutil.which('node'):
   run_check(['node','--check','assets/v315/intelligence.js'],repo);run_check(['node','tests/ui_smoke_v315.js'],repo)
 ...
 shutil.rmtree(backup,ignore_errors=True);print(f'v3.15.0 instalada y validada: {len(files)} archivos. .git preservado. No se ha creado commit ni push.');return 0
