...
from v35.build_intelligence import build as build_public_intelligence
...
def run(root: Path, profile: str = "daily", foundation_rc: int = 0) -> dict[str, Any]:
    ...
    # Rebuild only descriptive intelligence foundations. The v3.5 pipeline deliberately
    ...
    write_json(out34 / "architectures.json", architectures)
    ...
    out35 = root / "data/v35"
    out35.mkdir(parents=True, exist_ok=True)
    write_json(out35 / "intelligence.json", public)
    ...
    write_json(out35 / "last_run.json", result)
    return result
