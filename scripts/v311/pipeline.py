...
from v311.build_intelligence import build as build_public_intelligence
...
def _merged_source_expansion(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    additions=[load_json(root/p,{}) for p in ('config/v35/source_additions.json','config/v36/source_additions.json','config/v38/source_additions.json','config/v39/source_additions.json')]
    merged_sources={}
...
def run(root: Path, profile: str='daily', foundation_rc: int=0) -> dict[str, Any]:
    ...
    public=build_public_intelligence();out=root/'data/v311';out.mkdir(parents=True,exist_ok=True);write_json(out/'intelligence.json',public)
    gaps=load_json(root/'data/v39/research_gaps.json',{}) or load_json(root/'data/v38/research_gaps.json',{}) or {};write_json(out/'research_gaps.json',{**gaps,'version':'3.11.0','note':'v3.11 hereda huecos públicos y refuerza clasificación de mayoristas.'})
    finished=datetime.now(timezone.utc);cleanup=public.get('meta',{}).get('distribution_cleanup',{}) or {}
    result={'version':'3.11.0','profile':profile,'status':'published','started_at':started.isoformat(),'finished_at':finished.isoformat(),'runtime_seconds':round((finished-started).total_seconds(),3),'foundation_rc':foundation_rc,'manufacturers':len(public.get('manufacturers',[])),'distributors':len(public.get('distributors',[])),'integrators':len(public.get('integrators',[])),'clients':len(public.get('clients_public',[]))+len(public.get('clients_private',[])),'clients_public':len(public.get('clients_public',[])),'clients_private':len(public.get('clients_private',[])),'trends':len(public.get('trends',[])),'architectures':len(public.get('architectures',[])),'source_count':len(public.get('source_catalog',[])),'research_gaps':gaps.get('total_gaps',0),'high_priority_research_gaps':gaps.get('high_priority_gaps',0),'traceable_fields':sum(1 for section in ('manufacturers','distributors','integrators','clients_public','clients_private','trends','architectures') for row in public.get(section,[]) for value in (row.get('fields') or {}).values() if value and value.get('evidence')),'distribution_cleanup':cleanup,'obsolete_decision_outputs_removed':removed}
