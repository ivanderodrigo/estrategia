...
def validate(root: Path) -> list[str]:
    errors: list[str] = []
    intelligence_path = root / "data/v36/intelligence.json"
    if not intelligence_path.exists():
        return ["falta data/v36/intelligence.json"]
    try:
    ...
    required_trend_fields = {"observed","horizon","drivers","buyer_priorities","westcon_vendors","evolution","sources"}
    ...
    metric_coverage = sum("market_metrics" in (row.get("fields") or {}) for row in data.get("trends") or [])
    ...
    js_path = root / "assets/v360/intelligence.js"
    css_path = root / "assets/v360/intelligence.css"
    ...
    if index_path.exists():
        ...
        if "assets/v360/intelligence.js" not in index or "assets/v360/intelligence.css" not in index:
            errors.append("index.html no carga exclusivamente la capa v3.6 esperada")
