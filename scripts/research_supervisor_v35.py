...
from v35.pipeline import run
from v35.validate_v35 import validate

PROFILE_MAP = {"daily": "daily", "weekly": "weekly", "deep": "weekly", "monthly": "monthly", "exhaustive": "monthly"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Westcon Iberia Business Intelligence v3.5 supervisor")
    ...
    parser.add_argument("--skip-v33", action="store_true", help="Reutiliza la última investigación base y reconstruye solo la capa descriptiva v3.5")
...
def main() -> int:
    ...
    if not args.skip_v33:
        ...
        print(f"v3.5 foundation · investigación {canonical} hasta {foundation_runtime}s", flush=True)
        try:
            ...
            foundation_rc = 124
        if foundation_rc:
            print("v3.5 warning · la investigación base no terminó correctamente; se conserva la última evidencia válida y continúa la publicación descriptiva", flush=True)

    print("v3.5 · reconstruyendo inteligencia trazable de fabricantes, integradores, mayoristas, tendencias y arquitecturas", flush=True)
    result = run(ROOT, canonical, foundation_rc=foundation_rc)
    errors = validate(ROOT)
    if errors:
        print("v3.5 validation failed · " + "; ".join(errors[:16]), file=sys.stderr, flush=True)
        return 1
    print(
        f"v3.5.0 published · fabricantes {result['manufacturers']} · integradores {result['integrators']} · "
