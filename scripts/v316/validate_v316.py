...
def validate(root:Path|None=None):
 ...
 try:
  data=json.loads((base/'data/v316/intelligence.json').read_text(encoding='utf-8'));run=json.loads((base/'data/v316/last_run.json').read_text(encoding='utf-8'));gaps=json.loads((base/'data/v316/research_gaps.json').read_text(encoding='utf-8'));metrics=json.loads((base/'data/v316/metrics_before_after.json').read_text(encoding='utf-8'))
 except Exception as exc:return [f'No se pueden cargar los artefactos v316: {exc}']
 check((base/'VERSION').read_text(encoding='utf-8').strip()=='3.16.0','VERSION no es 3.16.0');check(data.get('meta',{}).get('version')=='3.16.0','dataset no es 3.16.0');check(run.get('version')=='3.16.0','last_run no es 3.16.0');check(gaps.get('version')=='3.16.0','gaps no es 3.16.0')
 ...
 for name in ('metrics_before_after.json','source_report.json','coverage_report.json','research_gaps.json'):check((base/'data/v316'/name).exists(),f'Falta {name}')
 index=(base/'index.html').read_text(encoding='utf-8');js=(base/'assets/v316/intelligence.js').read_text(encoding='utf-8')
 check('assets/v316/intelligence.js?v=3.16.0' in index and 'assets/v316/intelligence.css?v=3.16.0' in index,'index no usa v3.16.0');check("fetch('data/v316/intelligence.json'" in js,'frontend no carga v316');check('Por investigar</span>' in js,'vacíos no muestran Por investigar');check('>—<' not in js,'guion usado como dato pendiente');check("let cols=schema.filter(col=>col.hidden!==true)" in js,'columnas esperadas pueden ocultarse por dispersión');check('fact_confidence' in js and 'interpretation_confidence' in js and 'action_risk' in js,'confianzas no separadas')
 ...
 for name in ('research-daily.yml','research-weekly.yml','research-monthly.yml'):
  text=(base/'.github/workflows'/name).read_text(encoding='utf-8');check('research_supervisor_v316.py' in text and 'tests/test_v316.py' in text and 'data/v316/' in text,f'{name} no usa v316')
 check('cp -R data/v316 _site/data/v316' in (base/'.github/workflows/pages-deploy.yml').read_text(encoding='utf-8'),'Pages no publica v316')
 for name in ('README_V316.md','CHANGELOG_V316.md'):check((base/name).exists(),f'Falta {name}')
 return errors
def main():
 errors=validate(ROOT)
 if errors:raise SystemExit('VALIDACIÓN v3.16.0 · FAIL · '+'; '.join(errors[:40]))
 print('VALIDACIÓN v3.16.0 · PASS')
