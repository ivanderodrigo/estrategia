"""Exhaustive 32-pass queue for every value or evidence gap."""
...
def research_plan(section:str,row:Mapping[str,Any],field_id:str)->list[dict[str,Any]]:
 ...
 return [{'pass':i,'strategy':PASS_NAMES[i-1],'query':_query(entity,field_id,i,scope),'languages':['es','pt','en'],'source_family':PASS_NAMES[i-1],'objective':'confirm' if i<31 else ('corroborate' if i==31 else 'challenge')} for i in range(1,33)]
def build_gaps(public:Mapping[str,Any],version:str='3.16.0')->dict[str,Any]:
 ...
 return {'version':version,'generated_at':datetime.now(timezone.utc).isoformat(),'definition':'Misma definición v3.15→v3.16: todo campo declarado cuenta; un valor solo cierra el gap con evidencia suficiente y sin ocultar opcionales.','total_gaps':len(gaps),'critical_gaps':sum(critical.values()),'high_priority_gaps':sum(critical.values()),'by_section':by_section,'critical_by_section':dict(critical),'missing_by_field':{f'{s}.{f}':n for (s,f),n in missing.most_common()},'research_states':dict(states),'coverage':coverage,'engine':{'passes_per_gap':32,'languages':['es','pt','en'],'timeouts_seconds':25,'retries':4,'backoff':'exponential','circuit_breaker_failures':5,'per_domain_concurrency':2,'global_concurrency':12,'checkpoint_every_results':20,'incremental':True,'resume':True,'failure_isolation':'per-source','contradiction_pass':True,'learning_log':'data/v316/research_learning.json'},'gaps':gaps}
