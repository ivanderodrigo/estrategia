#!/usr/bin/env python3
"""Build v3.16: deeper gap research and an explicit evidence traffic light."""
...
from v316.gap_engine import build_gaps,evidence_rows,has_value
from v316.metrics import calculate_metrics,compare_metrics
from v34.common import write_json
VERSION='3.16.0';SECTIONS=('manufacturers','distributors','integrators','clients_public','clients_private','trends','architectures')
...
def apply_deep_evidence(data:dict[str,Any])->Counter:
 cfg=load_json('config/v316/deep_evidence.json',{});counts=Counter()
...
def enrich_source_catalog(data:dict[str,Any])->int:
 rows={str(old.get('id') or old.get('source_id')):dict(old) for old in data.get('source_catalog') or [] if old.get('id') or old.get('source_id')};before=len(rows);cfg=load_json('config/v316/deep_evidence.json',{})
 for group in ('clients','distributors','integrators'):
  for raw in cfg.get(group) or []:
   source=raw.get('source') or {};url=str(source.get('url') or '');sid='v316-'+canonical(source.get('source') or source.get('title')).replace(' ','-')[:70]
...
def build()->dict[str,Any]:
 data=copy.deepcopy(build_v315());baseline=copy.deepcopy(data);now=datetime.now(timezone.utc).isoformat();configure_schema(data);counts=apply_deep_evidence(data);derive_integrator_fields(data,counts);derive_distributor_fields(data,counts);derive_client_fields(data,counts);derive_manufacturer_signals(data,counts);new_sources=enrich_source_catalog(data);distribution=regrade_all(data)
 data.setdefault('meta',{}).update({'version':VERSION,'generated_at':now,'research_model':'gap-driven-32-pass-multi-evidence','claim_model':'fact-signal-interpretation-red-yellow-green','scope':'España y Portugal','source_count':len(data.get('source_catalog') or []),'v316_research':{**dict(counts),'new_source_routes':new_sources,'researched_at':'2026-08-30','passes_per_gap':32,'languages':['es','pt','en'],'confidence_distribution':dict(distribution)},'traceability':'Verde = fuente oficial o corroboración fuerte; amarillo = evidencia parcial o interpretación; rojo = señal o indicio. Las señales nunca se publican como hechos.'});data['_baseline_v315']=baseline;return data
def write_snapshot(data:dict[str,Any])->dict[str,Any]:
 baseline=data.pop('_baseline_v315');out=ROOT/'data/v316';out.mkdir(parents=True,exist_ok=True);gaps=build_gaps(data,version=VERSION);before_gaps=build_gaps(baseline,version='3.15.0');before=calculate_metrics(baseline,before_gaps,version='3.15.0');after=calculate_metrics(data,gaps,version=VERSION);comparison=compare_metrics(before,after,baseline,data)
 write_json(out/'intelligence.json',data);write_json(out/'research_gaps.json',gaps);write_json(out/'metrics_before_after.json',comparison);write_json(out/'source_report.json',{'version':VERSION,'catalog_sources':len(data.get('source_catalog') or []),'unique_evidence_sources':after['unique_sources'],'unique_domains':after['unique_domains'],'official_evidence':after['official_evidence'],'new_routes':data['meta']['v316_research']['new_source_routes'],'confidence_distribution':data['meta']['v316_research']['confidence_distribution'],'families':sorted({str(s.get('class') or 'unknown') for s in data.get('source_catalog') or []})});write_json(out/'coverage_report.json',{'version':VERSION,'sections':after['sections'],'gap_by_section':after['gap_by_section']})
 result={'version':VERSION,'profile':'snapshot','status':'published','started_at':data['meta']['generated_at'],'finished_at':datetime.now(timezone.utc).isoformat(),**{k:after[k] for k in ('manufacturers','distributors','integrators','clients','clients_public','clients_private','trends','architectures','sources','evidence','relationships','traceable_fields','research_gaps','critical_gaps','unique_sources','unique_domains','official_evidence','corroborated_evidence','populated_fields','overall_completeness_pct')},'gap_by_section':after['gap_by_section'],'sections':after['sections'],'new_information':comparison['new_information'],'research_engine':gaps['engine'],'v316_research':data['meta']['v316_research']};write_json(out/'last_run.json',result);return result
