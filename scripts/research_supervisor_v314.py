...
def main():
    ...
    if not args.skip_v33:
        ...
        print(f'v3.14 foundation · investigación {canonical} hasta {foundation}s · fuentes ampliadas',flush=True)
        ...
        if foundation_rc:print('v3.14 warning · se conserva la última evidencia pública válida y continúa la reconstrucción',flush=True)
    print('v3.14 · reconstruyendo mayoristas validados + ecosistema fabricante↔integrador + IBEX/PSI + contratación exacta',flush=True)
    result=run(ROOT,args.profile,foundation_rc=foundation_rc);errors=validate(ROOT)
    if errors:print('v3.14 validation failed · '+'; '.join(errors[:20]),file=sys.stderr,flush=True);return 1
    graph=result.get('integrator_graph') or {};dist=result.get('distributor_validation') or {};proc=result.get('public_procurement') or {}
    print(f"v3.14.0 published · fabricantes {result['manufacturers']} · mayoristas validados {result['distributors']} · integradores {result['integrators']} · relaciones fabricante↔integrador {graph.get('unique_vendor_integrator_edges',0)} · clientes {result['clients']} · pliegos exactos {proc.get('exact_notices',0)} · fuentes {result['source_count']} · descartados no validados {len(dist.get('removed_unvalidated') or [])}",flush=True);return 0
