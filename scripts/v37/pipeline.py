...
from v34.intelligence_outputs import (
...
from v37.build_intelligence import build as build_public_intelligence
from v37.gap_engine import write_gaps
...
def _merged_source_expansion(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    v35_additions = load_json(root / "config/v35/source_additions.json", {})
    v36_additions = load_json(root / "config/v36/source_additions.json", {})
    v37_additions = load_json(root / "config/v37/source_additions.json", {})

    merged_sources: dict[str, dict[str, Any]] = {}
    for source in [
        *(source_expansion.get("sources") or []),
        *(audience_routes.get("sources") or []),
        *(v35_additions.get("sources") or []),
        *(v36_additions.get("sources") or []),
        *(v37_additions.get("sources") or []),
    ]:
...
def run(root: Path, profile: str = "daily", foundation_rc: int = 0) -> dict[str, Any]:
    ...
    # Rebuild only descriptive intelligence foundations. The v3.6 pipeline deliberately
    # does not import or execute the previous action/recommendation engine.
    ...
    # v3.6 public architectures are analyst/standards-led and are built in v36;
    # the old domain/materiality matcher is deliberately not executed.
    legacy_arch = out34 / "architectures.json"
    if legacy_arch.exists():
        legacy_arch.unlink()
    # Internal learning queue: intentionally not exposed by the user interface.
    ...
    out36 = root / "data/v37"
    out36.mkdir(parents=True, exist_ok=True)
    write_json(out36 / "intelligence.json", public)
    gaps = write_gaps(root, public)
    ...
    result = {
        ...
        "research_gaps": gaps.get("total_gaps", 0),
    ...
    write_json(out36 / "last_run.json", result)
    return result
