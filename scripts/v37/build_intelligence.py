...
CONFIDENCE_POLICY = json.loads((ROOT / "config/v37/confidence_policy.json").read_text(encoding="utf-8"))
...
def build_manufacturers() -> list[dict[str, Any]]:
    ...
    v3.7 deliberately treats confidence as a display dimension rather than a
    ...
    curated_integrators = load("config/v37/curated_integrator_relations.json", {}).get("relations", []) or []
    curated_distributors = load("config/v37/curated_distributor_relations.json", {}).get("relations", []) or []
...
def build_ecosystem(kind: str) -> list[dict[str, Any]]:
    ...
    if kind == "integrator":
        signal_rows = [*signal_rows, *(load("config/v37/curated_integrator_relations.json", {}).get("relations", []) or [])]
    else:
        signal_rows = [*signal_rows, *(load("config/v37/curated_distributor_relations.json", {}).get("relations", []) or [])]
    ...
    for key, item in merged.items():
        ...
        for sig in item.get("signals") or []:
            ...
            if sig.get("role"):
                ...
                # v3.7: controlled semantic propagation. An explicitly published
                ...
                if ev and vendor_name:
                    ...
                    iev["method"] = "role-to-capability-inference-v3.7"
...
def build_trends() -> list[dict[str, Any]]:
    ...
    research = load("data/research.latest.json", {})
    enrich = load("config/v37/trend_enrichment.json", {})
    trend_state = load("config/v37/trend_state.json", {})
...
def build_architectures() -> list[dict[str, Any]]:
    ...
    """
    doc = load("config/v37/architecture_frameworks.json", {})
    ...
    for arch in doc.get("frameworks", []) or []:
        ...
        for layer in arch.get("layers", []) or []:
            ...
            for vendor_name in layer.get("vendors", []) or []:
                ...
                role_ev = {
                    "source": "Westcon Iberia Business Intelligence v3.7",
...
def merge_source_catalog() -> list[dict[str, Any]]:
    ...
    additions_v37 = load("config/v37/source_additions.json", {}).get("sources", []) or []
...
if __name__ == "__main__":
    data = build()
    write("data/v37/intelligence.json", data)
    write("data/v37/last_run.json", {
        "version": "3.7.0", "generated_at": data["meta"]["generated_at"], "status": "published",
    ...
    print(json.dumps(load("data/v37/last_run.json", {}), ensure_ascii=False))
