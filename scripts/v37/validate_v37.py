...
def validate(root: Path) -> list[str]:
    errors: list[str] = []
    intelligence_path = root / "data/v37/intelligence.json"
    if not intelligence_path.exists():
        return ["falta data/v37/intelligence.json"]
    try:
        data = _load(intelligence_path)
    except Exception as exc:
        return [f"intelligence.json inválido: {exc}"]

    if data.get("meta", {}).get("version") != "3.7.0":
        errors.append("la versión pública no es 3.7.0")
    ...
    js_path = root / "assets/v370/intelligence.js"
    css_path = root / "assets/v370/intelligence.css"
    ...
    if index_path.exists():
        ...
        if "assets/v370/intelligence.js" not in index or "assets/v370/intelligence.css" not in index:
            errors.append("index.html no carga exclusivamente la capa v3.7 esperada")
