...
from v312.build_intelligence import build as build_public_intelligence
from v312.procurement_research import run as refresh_procurement
...
def _merged_source_expansion(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    additions=[load_json(root/p,{}) for p in (
        ...
        'config/v39/source_additions.json','config/v312/source_additions.json'
    )]
    merged_sources={}
...
def run(root: Path, profile: str='daily', foundation_rc: int=0) -> dict[str, Any]:
    ...
    public=build_public_intelligence();out=root/'data/v312';out.mkdir(parents=True,exist_ok=True);write_json(out/'intelligence.json',public)
    gaps=load_json(root/'data/v39/research_gaps.json',{}) or load_json(root/'data/v38/research_gaps.json',{}) or {}
    graph=(public.get('meta') or {}).get('integrator_graph') or {}
    write_json(out/'research_gaps.json',{**gaps,'version':'3.12.0','note':'v3.12 prioriza ecosistema fabricante↔integrador, mayoristas validados, contratación exacta y cobertura IBEX 35/PSI.','v312_priority_manufacturers':graph.get('manufacturers_below_3_integrators',[])})
    finished=datetime.now(timezone.utc)
    meta=public.get('meta',{})
    result={
        'version':'3.12.0','profile':profile,'status':'published','started_at':started.isoformat(),'finished_at':finished.isoformat(),
