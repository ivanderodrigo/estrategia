...
from v314.build_intelligence import build as build_public_intelligence
from v314.procurement_research import run as refresh_procurement
...
def _merged_source_expansion(root: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    additions=[load_json(root/p,{}) for p in (
        ...
        'config/v39/source_additions.json','config/v313/source_additions.json','config/v314/source_additions.json'
    )]
    merged_sources={}
...
def run(root: Path, profile: str='daily', foundation_rc: int=0) -> dict[str, Any]:
    ...
    public=build_public_intelligence();out=root/'data/v314';out.mkdir(parents=True,exist_ok=True);write_json(out/'intelligence.json',public)
    gaps=build_gaps(public)
    graph=(public.get('meta') or {}).get('integrator_graph') or {}
    gaps['note']='v3.14 separa huecos decisionales de enriquecimiento opcional y mantiene rutas oficiales/recíprocas para los gaps críticos.'
    gaps['v314_priority_manufacturers']=graph.get('manufacturers_below_3_integrators',[])
    write_json(out/'research_gaps.json',gaps)
    finished=datetime.now(timezone.utc)
    meta=public.get('meta',{})
    result={
        'version':'3.14.0','profile':profile,'status':'published','started_at':started.isoformat(),'finished_at':finished.isoformat(),
        ...
        'distributor_validation':meta.get('distributor_validation',{}),'distributor_business_enrichment':meta.get('distributor_business_enrichment',{}),'portfolio_fit_cleanup':meta.get('portfolio_fit_cleanup',{}),
